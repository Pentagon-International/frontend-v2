...
import { IconPencil, IconPencilFilled } from "@tabler/icons-react";
...
type FileStatus =
  "pending" | "processing" | "done" | "failed" | "job_created" | string;
...
const STYLES = `
  ...
  .hbl-app .topbar { background: var(--surface); border-bottom: 1px solid var(--border); padding: 10px 20px; display: flex; align-items: center; gap: 14px; min-height: 54px; flex-wrap: wrap; border-radius: 1rem 1rem 0 0; flex-shrink: 0; }
  .hbl-app .top-brand { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
  ...
  .hbl-app .top-brand-text { font-size:.95rem; font-weight:700; letter-spacing:-.02em; color:var(--text); }
  .hbl-app .top-brand-sub { font-size: .65rem; color: var(--muted); font-family: var(--mono); }
  .hbl-app .top-divider { width: 1px; height: 24px; background: var(--border); flex-shrink: 0; }
  .hbl-app .search-wrap { position: relative; flex: 1 1 220px; min-width: 180px; max-width: 380px; }
  .hbl-app .top-actions { display: flex; gap: 8px; align-items: center; margin-left: auto; flex-wrap: wrap; justify-content: flex-end; }
  .hbl-app .top-stats { font-family: var(--mono); font-size: .68rem; color: var(--muted); display: flex; gap: 12px; white-space: nowrap; }
  ...
  .hbl-app .filter-bar { background: var(--surface); border-bottom: 1px solid var(--border); padding: 8px 20px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; flex-shrink: 0; }
  ...
  .hbl-app .table-wrap { background: var(--bg); flex: 1; min-height: 0; overflow: auto; }
  .hbl-app .data-table { width: 100%; min-width: 960px; border-collapse: collapse; table-layout: fixed; background: var(--surface); }
  .hbl-app .data-table col.col-check { width: 44px; }
  .hbl-app .data-table col.col-txn { width: 118px; }
  .hbl-app .data-table col.col-file { width: 22%; }
  .hbl-app .data-table col.col-bl { width: 130px; }
  .hbl-app .data-table col.col-status { width: 132px; }
  .hbl-app .data-table col.col-user { width: 120px; }
  .hbl-app .data-table col.col-pdf { width: 90px; }
  .hbl-app .data-table col.col-date { width: 140px; }
  .hbl-app .data-table col.col-actions { width: 128px; }
  .hbl-app .data-table thead th { position: sticky; top: 0; z-index: 2; background: var(--surface2); text-align: left; padding: 10px 12px; font-size: .62rem; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: var(--muted); border-bottom: 1px solid var(--border); white-space: nowrap; cursor: pointer; user-select: none; vertical-align: middle; }
  .hbl-app .data-table thead th:first-child,
  .hbl-app .data-table tbody td:first-child { padding-left: 16px; text-align: left; }
  .hbl-app .data-table thead th:last-child,
  .hbl-app .data-table tbody td:last-child { padding-right: 16px; }
  .hbl-app .data-table thead th:hover { color: var(--text); }
  .hbl-app .data-table tbody tr { border-bottom: 1px solid var(--border); transition: background .1s; cursor: pointer; }
  .hbl-app .data-table tbody tr:hover { background: #f8f9fc; }
  .hbl-app .data-table tbody tr.selected { background: rgba(37,99,235,.05); }
  .hbl-app .data-table tbody td { padding: 10px 12px; font-size: .8rem; color: var(--muted); vertical-align: middle; text-align: left; overflow: hidden; }
  .hbl-app .data-table td input[type=checkbox],
  .hbl-app .data-table th input[type=checkbox] { accent-color: var(--accent); width: 14px; height: 14px; cursor: pointer; margin: 0; display: block; }
  .hbl-app .td-txn { font-family: var(--mono); font-size: .7rem; color: var(--accent); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .hbl-app .td-file { overflow: hidden; min-width: 0; }
  .hbl-app .td-filename { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--text); font-weight: 600; }
  .hbl-app .td-file-meta { font-size: .65rem; color: var(--muted); margin-top: 2px; font-family: var(--mono); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .hbl-app .td-bl { font-family: var(--mono); font-size: .72rem; color: var(--cyan); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .hbl-app .td-user { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .hbl-app .td-pdf { overflow: hidden; }
  .hbl-app .td-date { font-family: var(--mono); font-size: .68rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .hbl-app .td-status { overflow: hidden; }
  .hbl-app .badge { display: inline-flex; align-items: center; gap: 5px; max-width: 100%; min-width: 0; padding: 3px 8px; border-radius: 4px; font-size: .62rem; font-weight: 700; letter-spacing: .02em; }
  .hbl-app .badge::before { content: ''; width: 5px; height: 5px; border-radius: 50%; display: inline-block; flex-shrink: 0; }
  .hbl-app .badge-text { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; }
  ...
  .hbl-app .badge-job_created::before { background: #0ea5e9; }
  .hbl-app .badge-unknown { background: var(--surface2); color: var(--muted); border: 1px solid var(--border); }
  .hbl-app .badge-unknown::before { background: var(--muted); }
  ...
  .hbl-app .pdf-tag { display: inline-block; max-width: 100%; font-family: var(--mono); font-size: .6rem; color: var(--muted); border: 1px solid var(--border); border-radius: 3px; padding: 1px 5px; background: var(--surface2); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; vertical-align: middle; }
  ...
  .hbl-app .pagination { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; padding: 12px 20px; border-top: 1px solid var(--border); background: var(--surface); border-radius: 0 0 1rem 1rem; flex-shrink: 0; }
  ...
  .hbl-app .table-wrap > .state-box { min-height: 240px; }
  ...
  .hbl-app .action-bar { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%) translateY(80px); background: var(--surface); border: 1px solid var(--border2); border-radius: 10px; padding: 10px 16px; display: flex; align-items: center; gap: 12px; box-shadow: 0 8px 28px rgba(0,0,0,.13); transition: transform .25s cubic-bezier(.34,1.56,.64,1); z-index: 50; max-width: calc(100vw - 32px); flex-wrap: wrap; }
  ...
  .hbl-app .overlay { position: fixed; inset: 0; background: rgba(0,0,0,.3); backdrop-filter: blur(4px); z-index: 100; display: flex; align-items: center; justify-content: center; padding: 16px; }
  .hbl-app .modal { background: var(--surface); border: 1px solid var(--border2); border-radius: 14px; width: min(90%, 600px); max-height: min(88vh, 100%); display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,.15); animation: hbl-modal-in .2s ease; min-width: 0; overflow: hidden; }
  .hbl-app .modal-lg { width: min(90%, 820px); max-width: 820px; }
  .hbl-app .modal-xl { width: min(96%, 1100px); max-width: 1100px; max-height: min(92vh, 100%); }
  .hbl-app .modal-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 16px 20px; border-bottom: 1px solid var(--border); flex-wrap: wrap; min-width: 0; }
  ...
  .hbl-app .zone-file-row { display: flex; align-items: center; justify-content: space-between; gap: 8px; background: var(--surface2); border: 1px solid var(--border); border-radius: 6px; padding: 6px 8px; margin-top: 4px; }
  .hbl-app .zone-file-row > div:first-child { min-width: 0; flex: 1; }
  .hbl-app .zfname { font-size: .72rem; color: var(--text); font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; flex: 1; }
  ...
  .hbl-app .detail-fname { font-size: .95rem; font-weight: 700; overflow: hidden; text-overflow: ellipsis; min-width: 0; }
  .hbl-app .info-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
  ...
  .hbl-app .form-body { padding: 20px 24px 32px; overflow-y: auto; min-width: 0; }
  ...
  .hbl-app .fgrid { display: grid; gap: 10px; }
  .hbl-app .fgrid.c4 { grid-template-columns: repeat(4, minmax(0, 1fr)); }
  .hbl-app .fgrid.c3 { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .hbl-app .fgrid.c2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .hbl-app .fgrid > * { min-width: 0; }
  .hbl-app .ff { display: flex; flex-direction: column; gap: 3px; min-width: 0; }
  .hbl-app .ff.s2 { grid-column: span 2; }
  .hbl-app .ff.s3 { grid-column: span 3; }
  .hbl-app .ff.s4 { grid-column: 1 / -1; }
  .hbl-app .ff label { font-size: .57rem; font-weight: 700; text-transform: uppercase; letter-spacing: .1em; color: var(--muted); text-align: left; }
  .hbl-app .fval { font-size: .78rem; color: var(--text); background: var(--surface2); border: 1px solid var(--border); border-radius: 5px; padding: 5px 10px; min-height: 30px; word-break: break-word; line-height: 1.45; text-align: left; }
  ...
  .hbl-app .hcard-bl { font-family: var(--mono); font-size: .75rem; color: var(--text); font-weight: 600; min-width: 0; overflow: hidden; text-overflow: ellipsis; }
  .hbl-app .hcard-shipper { font-size: .72rem; color: var(--muted); min-width: 0; overflow: hidden; text-overflow: ellipsis; }
  ...
  .hbl-app .ctable-wrap { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }
  .hbl-app .ctable { width: 100%; min-width: 480px; border-collapse: collapse; margin-top: 6px; table-layout: auto; display: table; }
  .hbl-app .ctable thead { display: table-header-group; }
  .hbl-app .ctable tbody { display: table-row-group; }
  .hbl-app .ctable tr { display: table-row; }
  .hbl-app .ctable th, .hbl-app .ctable td { display: table-cell; text-align: left; vertical-align: middle; }
  .hbl-app .ctable th { font-size: .57rem; text-transform: uppercase; letter-spacing: .08em; color: var(--muted); font-weight: 700; padding: 7px 10px; background: var(--surface2); border: 1px solid var(--border); white-space: nowrap; cursor: default; }
  .hbl-app .ctable td { padding: 7px 10px; border: 1px solid var(--border); color: var(--text); font-family: var(--mono); font-size: .7rem; word-break: break-word; }
  ...
  .hbl-app .upload-summary { display: flex; gap: 8px; margin-bottom: 12px; padding: 10px 12px; background: var(--surface2); border: 1px solid var(--border); border-radius: 8px; align-items: center; flex-wrap: wrap; }
  ...
  .hbl-app .port-fix-row { display: flex; gap: 10px; align-items: flex-start; width: 100%; min-width: 0; }
  .hbl-app .port-fix-row > div { min-width: 0; }
  @media (max-width: 1100px) {
    .hbl-app .fgrid.c4 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    .hbl-app .ff.s3 { grid-column: span 2; }
  }
  @media (max-width: 768px) {
    .hbl-app .info-grid, .hbl-app .upload-zones { grid-template-columns: 1fr; }
    .hbl-app .fgrid.c4, .hbl-app .fgrid.c3, .hbl-app .fgrid.c2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    .hbl-app .ff.s2, .hbl-app .ff.s3, .hbl-app .ff.s4 { grid-column: 1 / -1; }
    .hbl-app .top-divider { display: none; }
    .hbl-app .search-wrap { max-width: none; flex: 1 1 100%; }
    .hbl-app .top-actions { margin-left: 0; width: 100%; }
    .hbl-app .form-body { padding: 14px 12px 24px; }
    .hbl-app .tab-body, .hbl-app .modal-body { padding: 14px 12px; }
    .hbl-app .hcard-mode { margin-left: 0; }
    .hbl-app .port-fix-row { flex-direction: column; }
    .hbl-app .port-fix-row > div:first-child { flex: 1 1 auto !important; width: 100%; }
  }
  @media (max-width: 560px) {
    .hbl-app .fgrid.c4, .hbl-app .fgrid.c3, .hbl-app .fgrid.c2 { grid-template-columns: minmax(0, 1fr); }
    .hbl-app .filter-bar { padding: 8px 12px; }
    .hbl-app .topbar { padding: 10px 12px; }
    .hbl-app .pagination { padding: 10px 12px; }
    .hbl-app .cost-cell-box { min-width: 0; flex: 1 1 100%; }
  }
`;
...
const Sec: FC<{ icon: string; title: string; children: ReactNode }> = ({
  icon,
  title,
  children,
}) => (
...
const SubSec: FC<{ icon: string; title: string; children: ReactNode }> = ({
  icon,
  title,
  children,
}) => (
...
const STATUS_BADGE_META: Record<string, { label: string; tone: string }> = {
  pending: { label: "Pending", tone: "pending" },
  processing: { label: "Processing", tone: "processing" },
  done: { label: "Done", tone: "done" },
  failed: { label: "Failed", tone: "failed" },
  job_created: { label: "Job Created", tone: "job_created" },
};
...
const statusBadgeLabel = (status?: string) => {
  ...
  if (!raw) return "—";
  const meta = STATUS_BADGE_META[statusKey(raw)];
...
const Badge: FC<{ status: FileStatus }> = ({ status }) => {
  ...
  const tone = STATUS_BADGE_META[statusKey(status)]?.tone ?? "unknown";
  return (
    ...
    return (
      <div>
        ...
        <div
          ...
          onClick={() =>
            (document.getElementById(`fi-${type}`) as HTMLInputElement)?.click()
          }
          onDragOver={(e) => {
            e.preventDefault();
            setDragOver(type);
        ...
        {files.length ? (
          files.map((f, i) => (
            <div className="zone-file-row" key={i}>
              ...
              <button className="zf-rm" onClick={() => removeFile(type, i)}>
                ✕
              </button>
            </div>
          ))
          ...
          {total > 0 && (
            <div className="upload-summary">
              <div className="us-item">
                HBL: <span className="us-count hbl">{hblFiles.length}</span>
              </div>
              <div
                style={{
                  width: 1,
              ...
              />
              <div className="us-item">
                MBL: <span className="us-count mbl">{mblFiles.length}</span>
              </div>
              <div
                style={{
                  width: 1,
            ...
            <div>
                ...
                <div className="ur-item ok" key={i}>
                  ...
                  <div>
                    <div className="ur-name">
                      {f.filename} <FtTag type={f.file_type} />
                    </div>
                    <div className="ur-meta">
                      {f.size_kb} KB · {f.pdf_type ?? "detecting..."} · Queued
                    </div>
              ...
              {results.error && (
                <div className="ur-item err">
                  <span>❌</span>
                  <div>
                    <div className="ur-name">Upload failed</div>
                  </div>
                </div>
              )}
            </div>
                ...
                <div className="file-section">
                  <div className="file-grid">
                    {hblSlots.map(({ label, file }) => (
                      <div
                        key={`${label}-${file.file_url}`}
                        className="file-grid-cell"
                      >
...
const DetailModal: FC<{
  ...
  onClose: () => void;
}> = ({ record, ports, carriers, containerTypes, onClose }) => {
        ...
        <div className="tab-body">
          ...
          {tab === "raw" &&
            (Object.keys(p).length ? (
              <pre style={{ maxHeight: "calc(88vh - 200px)" }}>
                {JSON.stringify(p, null, 2)}
              </pre>
            ) : (
              <div className="state-box">
                <div className="state-icon">📭</div>
                <div className="state-text">No payload data</div>
              </div>
            ))}
        </div>
      </div>
...
const PayloadFormBody: FC<{
  ...
  const HF = (
    ...
    onHousingFieldChange ? (
      <EF
        ...
        cls={cls}
        span={span}
        onChange={(v) => onHousingFieldChange(idx, field, v)}
      />
    ) : (
      <FF label={label} val={val} cls={cls} span={span} />
    );

  const resolvePort = (code?: string) =>
    ports.find((pt) => pt.alias_code === code || pt.port_code === code);
  const resolveCarrier = (code?: string) =>
    carriers.find((c) => c.alias_code === code || c.carrier_code === code);
  const resolveContainerType = (code?: string) =>
    containerTypes.find(
      (ct) => ct.alias_code === code || ct.container_code === code,
    );

  const PortCodeView: FC<{
    label: string;
    code?: string;
    span?: 1 | 2 | 3 | 4;
    onFix?: () => void;
  }> = ({ label, code, span = 1, onFix }) => {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          {code && (
            <div
              style={{
                fontSize: ".68rem",
                color: isValid ? "var(--muted)" : "var(--red)",
                fontFamily: "var(--sans)",
                paddingLeft: 2,
              }}
            >
              {isValid
                ? `${match.port_name} · ${match.transport_mode}`
                : "⚠ Not found in port list"}
            </div>
          )}
          {!isValid && code && onFix && (
            <button
  ...
  const CarrierCodeView: FC<{ label: string; code?: string }> = ({
    label,
    code,
  }) => {
  ...
  const ContainerTypeView: FC<{ code?: string; onFix?: () => void }> = ({
    code,
    onFix,
  }) => {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
        <div
          className="fval mono"
          style={{
            color: code ? (isValid ? "var(--green)" : "var(--red)") : undefined,
          }}
        >
          {code ?? "—"}
        </div>
        {code && (
          ...
          >
            {isValid
              ...
              : "⚠ Not found in container type list"}
          </div>
        )}
        {!isValid && code && onFix && (
          <button
    ...
    <div className="form-body">
      <Sec icon="📄" title="Document Header">
        <div className="fgrid c4">
          ...
          <CarrierCodeView
            label="Carrier Code"
            code={p.carrier_code as string | undefined}
          />
      ...
      <Sec icon="🗺️" title="Routing & Ports">
          ...
          {editPorts ? (
            <>
              <div className="ff" style={{ overflow: "visible" }}>
                <PortSearch
                  label="Origin Code"
                  value={editPorts.originCode}
                  inputText={editPorts.originText}
                  ports={editPorts.ports}
                  onChange={editPorts.onOriginChange}
                  onSelect={editPorts.onOriginSelect}
                  onFix={onFixCode}
                />
              </div>
              <div className="ff" style={{ overflow: "visible" }}>
                <PortSearch
                  ...
                  value={editPorts.destCode}
                  inputText={editPorts.destText}
                  ports={editPorts.ports}
                  onChange={editPorts.onDestChange}
                  onSelect={editPorts.onDestSelect}
                  onFix={onFixCode}
                />
              </div>
            </>
            ...
            <>
              <PortCodeView
                label="Origin Code"
                code={p.origin_code as string | undefined}
                onFix={onFixCode}
              />
              <PortCodeView
                label="Destination Code"
                code={p.destination_code as string | undefined}
                onFix={onFixCode}
              />
            </>
          )}
        ...
        {p.ocean_routings?.map((r, i) => (
          <div className="routing-pill" key={i}>
            ...
            <span className="rm">
              Vessel: {r.vessel ?? "—"} · Voyage: {r.voyage_number ?? "—"} ·{" "}
              {r.transport_type ?? ""}
            </span>
          </div>
        ))}
      ...
      <Sec icon="🚢" title="Vessel & Voyage">
        <div className="fgrid c4">
          ...
          {F(
            "On Board Date",
            p.on_board_date ?? p.shipped_on_board_date,
            "on_board_date",
          )}
      ...
      {Array.isArray(p.container_details) && p.container_details.length > 0 && (
        <Sec
          icon="📦"
          title={`Container Details (${p.container_details.length})`}
        >
                ...
                <tr key={i}>
                  ...
                  <td style={{ overflow: "visible" }}>
                    {onContainerDetailChange ? (
                      ...
                      <ContainerTypeView
                        code={c.container_type_input ?? c.container_type}
                        onFix={onFixContainerType}
                      />
                    )}
                  </td>
      ...
      {(onFieldChange || fv(p.special_clauses)) && (
        <Sec icon="📋" title="Special Clauses">
          <div className="fgrid c2">
            {F("Clause", p.special_clauses, "special_clauses", "", 2)}
          </div>
        </Sec>
      )}
      ...
      <Sec
        icon="🟣"
        title={`Housing Details — HBL Sub-bills (${housing.length})`}
      >
        {!housing.length ? (
          ...
          housing.map((hd, idx) => (
            <div className="hcard" key={idx}>
              <div className="hcard-head">
                ...
                {hd.cargo_mode && (
                  <span className="hcard-mode">{hd.cargo_mode}</span>
                )}
              </div>
              <div className="hcard-body">
                  ...
                  {editHousingPorts?.[idx] ? (
                    <>
                      <div className="ff" style={{ overflow: "visible" }}>
                        <PortSearch
                          label="Origin Code"
                          value={editHousingPorts[idx].originCode}
                          inputText={editHousingPorts[idx].originText}
                          ports={editHousingPorts[idx].ports}
                          onChange={editHousingPorts[idx].onOriginChange}
                          onSelect={editHousingPorts[idx].onOriginSelect}
                          onFix={
                            onFixHousingCode
                              ? () => onFixHousingCode(idx)
                              : undefined
                          }
                        />
                      </div>
                      <div className="ff" style={{ overflow: "visible" }}>
                        <PortSearch
                          label="Destination Code"
                          value={editHousingPorts[idx].destCode}
                          inputText={editHousingPorts[idx].destText}
                          ports={editHousingPorts[idx].ports}
                          onChange={editHousingPorts[idx].onDestChange}
                          onSelect={editHousingPorts[idx].onDestSelect}
                          onFix={
                            onFixHousingCode
                              ? () => onFixHousingCode(idx)
                              : undefined
                    ...
                    <>
                      <PortCodeView
                        label="Origin Code"
                        code={hd.origin_code as string | undefined}
                        onFix={
                          onFixHousingCode
                            ? () => onFixHousingCode(idx)
                            : undefined
                      ...
                      <PortCodeView
                        label="Destination Code"
                        code={hd.destination_code as string | undefined}
                        onFix={
                          onFixHousingCode
                            ? () => onFixHousingCode(idx)
                            : undefined
                ...
                {(hd.total_packages ||
                  hd.total_volume_cbm ||
                  hd.total_gross_weight_kgs) && (
                  <div className="cost-strip" style={{ marginBottom: 12 }}>
                    {hd.total_packages && (
                      <div className="cost-cell-box">
                        <div className="ccv" style={{ color: "var(--cyan)" }}>
                          {hd.total_packages}
                        </div>
                        <div className="ccl">Total Packages</div>
                      </div>
                    )}
                    {hd.total_volume_cbm && (
                      <div className="cost-cell-box">
                        ...
                        <div className="ccl">Total Volume (CBM)</div>
                      </div>
                    )}
                    {hd.total_gross_weight_kgs && (
                      <div className="cost-cell-box">
                        ...
                        <div className="ccl">Gross Weight (KGS)</div>
                      </div>
                    )}
                  </div>
                )}
                ...
                <SubSec icon="📦" title="Consignee">
                  <div className="fgrid c4">
                    ...
                    {HF(
                      idx,
                      "GST",
                      hd.consignee_gst ?? hd.gst_no_consignee,
                      "consignee_gst",
                      "mono",
                    )}
                    {HF(
                ...
                <SubSec icon="🔔" title="Notify Party 1">
                  <div className="fgrid c4">
                    ...
                    {HF(
                      idx,
                      "Email",
                      hd.notify1_customer_email,
                      "notify1_customer_email",
                    )}
                    {HF(
                      idx,
                      "Phone",
                      hd.notify1_customer_phone,
                      "notify1_customer_phone",
                      "mono",
                    )}
                  </div>
                </SubSec>

                {(hd.notify2_customer_name ||
                  hd.notify2_customer_address ||
                  onHousingFieldChange) && (
                ...
                {Array.isArray(hd.cargo_details) &&
                  hd.cargo_details.length > 0 && (
                    <SubSec
                      icon="📫"
                      title={`Cargo / Container Details (${hd.cargo_details.length})`}
                    >
                      <div className="ctable-wrap">
                      <table className="ctable">
                        <thead>
                        ...
                        <tbody>
                          {hd.cargo_details.map((c, ci) => (
                            <tr key={ci}>
                              ...
                              <td>
                                {c.gross_weight_kgs ?? c.gross_weight ?? "—"}
                              </td>
...
const PayloadModal: FC<PayloadModalProps> = ({
  txn_id,
  record: inlineRecord,
  onClose,
  showToast,
  ...
  const [fixTarget, setFixTarget] = useState<{ record: FileRecord } | null>(
    null,
  );
  const [containerTypeFixTarget, setContainerTypeFixTarget] = useState<{
    record: FileRecord;
  } | null>(null);
  const [carrierFixTarget, setCarrierFixTarget] = useState<{
    record: FileRecord;
  } | null>(null);
  ...
  const [housingOriginTexts, setHousingOriginTexts] = useState<string[]>(() => {
    ...
    return Array.isArray(h)
      ? (h as HousingDetail[]).map((hd) => (hd.origin_code as string) ?? "")
      : [];
  });
  const [housingDestTexts, setHousingDestTexts] = useState<string[]>(() => {
    const h = initPayload().housing_details;
    return Array.isArray(h)
      ? (h as HousingDetail[]).map(
          (hd) => (hd.destination_code as string) ?? "",
        )
      : [];
  });
    ...
    hblApi
      ...
      .then((r) => {
        ...
        const pd: PayloadData =
          r.data.api_payload ?? r.data.extracted_data ?? {};
        setEditData(pd);
        setOriginText((pd.origin_code as string) ?? "");
        setDestText((pd.destination_code as string) ?? "");
        const housing = Array.isArray(pd.housing_details)
          ? (pd.housing_details as HousingDetail[])
          : [];
        setHousingOriginTexts(
          housing.map((hd) => (hd.origin_code as string) ?? ""),
        );
        setHousingDestTexts(
          housing.map((hd) => (hd.destination_code as string) ?? ""),
        );
        setLoading(false);
      })
  ...
  const reloadContainerTypes = useCallback(() => {
    hblApi
      ...
      .then((r) => {
        setContainerTypes(r.data.container_codes ?? []);
        onRefreshList?.();
      })
      .catch(() => {});
  }, [onRefreshList]);
  ...
  const onHousingFieldChange = useCallback(
    (idx: number, field: keyof HousingDetail, val: string) => {
      setIsDirty(true);
      setEditData((prev) => {
  ...
  const savePayload = async () => {
    setSaving(true);
    try {
      await hblApi.patch(`/files/${txn_id}/payload`, editData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken") || ""}`,
        },
      });
      ...
      >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              ...
              {editData.document_type && (
                <span className="pdf-tag">{editData.document_type}</span>
              )}
        ...
        <div className="tabs" style={{ padding: "8px 24px 0" }}>
          {payloadTabs.map((t) => (
            <button
              key={t.key}
              className={`tab-btn ${ptab === t.key ? "active" : ""}`}
          ...
          {loading ? (
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              <PayloadFormBody
                ...
                onFixContainerType={() =>
                  rec && setContainerTypeFixTarget({ record: rec })
                }
                ...
                editPorts={{
                  ...
                  onOriginChange: (t) => {
                    setOriginText(t);
                    onFieldChange("origin_code", "");
                  },
                  onOriginSelect: (code, display) => {
                    setOriginText(display);
                    onFieldChange("origin_code", code);
                  },
                  onDestChange: (t) => {
                    setDestText(t);
                    onFieldChange("destination_code", "");
                  },
                  onDestSelect: (code, display) => {
                    setDestText(display);
                    onFieldChange("destination_code", code);
                  },
                }}
                ...
                onFixCode={() =>
                  rec &&
                  setFixTarget({ record: { ...rec, api_payload: editData } })
                }
                onFixHousingCode={() =>
                  rec &&
                  setFixTarget({ record: { ...rec, api_payload: editData } })
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
              <pre style={{ maxHeight: "calc(92vh - 220px)" }}>
                {JSON.stringify(
                  rec?.api_payload ?? rec?.extracted_data ?? {},
                  null,
                  2,
                )}
              </pre>
...
const PortSearch: FC<{
  ...
  const matchesAlias = value
    ? ports.some((p) => p.alias_code === value || p.port_code === value)
    : false;
      ...
      {value && (
        ...
        >
          <span
            style={{
              fontSize: ".7rem",
              color: matchesAlias ? "var(--green)" : "var(--red)",
              fontFamily: "var(--mono)",
            }}
          >
            {matchesAlias ? "✓" : "⚠"} {value}
        ...
        >
          {filtered.map((p) => (
            <div
              ...
              onMouseDown={(e) => {
                e.preventDefault();
                onSelect(p.port_code, `${p.port_code} – ${p.port_name}`);
                setOpen(false);
              }}
              style={{
                padding: "8px 12px",
...
const ContainerTypeSearch: FC<{
  ...
  const isValid = !!containerTypes.find(
    (ct) => ct.alias_code === value || ct.container_code === value,
  );
  ...
  return (
    <div ref={wrapRef}>
      ...
      {value && (
        ...
        >
          {isValid
            ? `✓ ${containerTypes.find((ct) => ct.alias_code === value || ct.container_code === value)?.container_name ?? ""}`
            : "⚠ Not found in container type list"}
        </div>
      )}
      {!isValid && value && onFix && (
...
const PortFixModal: FC<{
  ...
  const payload =
    record.api_payload ?? record.extracted_data ?? ({} as PayloadData);

  const isInvalidPort = (code?: string) =>
    !!code && !ports.some((p) => p.alias_code === code || p.port_code === code);

  const invalidRows = useMemo(() => {
    const byCode = new Map<string, string[]>();
    const add = (label: string, code?: string) => {
      if (!code || !isInvalidPort(code)) return;
      const labels = byCode.get(code) ?? [];
      if (!labels.includes(label)) labels.push(label);
      byCode.set(code, labels);
    };
    add("Origin Code", payload.origin_code as string | undefined);
    add("Destination Code", payload.destination_code as string | undefined);
    const housing = Array.isArray(payload.housing_details)
      ? (payload.housing_details as HousingDetail[])
      : [];
    housing.forEach((hd, i) => {
      add(`HBL ${i + 1} Origin Code`, hd.origin_code);
      add(`HBL ${i + 1} Destination Code`, hd.destination_code);
    });
    return Array.from(byCode.entries()).map(([aliasCode, labels]) => ({
      aliasCode,
      labels,
    }));
  }, [payload, ports]);
  ...
  const save = async () => {
    const toSave = invalidRows.filter((row) => fixes[row.aliasCode]);
    ...
    try {
      ...
      await Promise.all(
        toSave.map((row) => {
          const portCode = fixes[row.aliasCode];
          const tm =
            ports.find((p) => p.port_code === portCode)?.transport_mode ??
            "SEA";
          return hblApi.post(
            "/port-aliases/",
            {
  ...
  const canSave = invalidRows.some((row) => fixes[row.aliasCode]);
      ...
      <div className="modal" style={{ maxWidth: 500 }}>
        ...
        >
          {invalidRows.length === 0 ? (
            ...
            >
              No invalid port codes found.
            </div>
          ) : (
            invalidRows.map((row) => (
              <PortFixRow
                key={row.aliasCode}
                sectionLabel={row.labels.join(" · ")}
                aliasCode={row.aliasCode}
                fixedCode={fixes[row.aliasCode] ?? ""}
                inputText={texts[row.aliasCode] ?? ""}
                ports={ports}
                onTextChange={(t) => {
                  setTexts((prev) => ({ ...prev, [row.aliasCode]: t }));
                  setFixes((prev) => ({ ...prev, [row.aliasCode]: "" }));
                }}
                onPortSelect={(code, display) => {
                  setFixes((prev) => ({ ...prev, [row.aliasCode]: code }));
                  setTexts((prev) => ({ ...prev, [row.aliasCode]: display }));
                }}
              />
            ))
          )}
...
const CarrierFixModal: FC<{
  ...
  const invalid = (code?: string) =>
    !!code &&
    !carriers.some((c) => c.alias_code === code || c.carrier_code === code);

  const lookupCarrierName = useCallback(
    (code?: string) => {
  ...
  const save = async () => {
    ...
    try {
      const tm =
        carriers.find((c) => c.carrier_code === fixedCode)?.transport_mode ??
        "SEA";
      await hblApi.post(
        "/carrier-aliases/",
        {
                  ...
                  >
                    {filtered.map((c) => (
                      <div
                        ...
                        onMouseDown={(e) => {
                          e.preventDefault();
                          setFixedCode(c.carrier_code);
                          setInputText(c.carrier_name);
                          setOpen(false);
                        }}
                        style={{
                          padding: "8px 12px",
...
const HBLDocumentManager: FC = () => {
  ...
  const hasCodeValidationError = (f: FileRecord): boolean => {
    ...
    const invalidPort = (code?: string) =>
      !!code &&
      !ports.some((p) => p.alias_code === code || p.port_code === code);
    const invalidCarrier = (code?: string) =>
      !!code &&
      !carriers.some((c) => c.alias_code === code || c.carrier_code === code);
    const invalidContainerType = (code?: string) =>
      !!code &&
      !containerTypes.some(
        (ct) => ct.alias_code === code || ct.container_code === code,
      );
    const containerIssue =
      Array.isArray(payload.container_details) &&
    ...
    return (
      invalidPort(origin) ||
      invalidPort(dest) ||
      invalidCarrier(carrier) ||
      containerIssue
    );
  };

  useEffect(() => {
    const inProgress = allFiles.some(
  ...
  const getPortName = (code?: string) => {
    if (!code) return "";
    return (
      ports.find((p) => p.alias_code === code || p.port_code === code)
        ?.port_name ?? ""
    );
  };
    ...
    .filter((f) => {
      ...
      const matchSearch =
        ...
        [
          ...
          originCode,
          destCode,
          getPortName(originCode),
          getPortName(destCode),
  ...
  const filterSet = (s: FileStatus | "") => {
    setStatusFilter(s);
    setPage(1);
    setSelected(new Set());
  };
  const searchSet = (v: string) => {
    setSearch(v);
    setPage(1);
  };
  const toggleRow = (txn: string, checked: boolean) =>
    setSelected((prev) => {
      const s = new Set(prev);
      checked ? s.add(txn) : s.delete(txn);
      return s;
    });
  const toggleAll = (checked: boolean) =>
    setSelected(new Set(checked ? pageRows.map((f) => f.txn_id) : []));
  ...
  const startJobs = async (ids: string[]) => {
    ...
    if (invalid.length) {
      showToast(
        `❌ ${invalid.length} record(s) have invalid port/carrier/container type codes. Fix them before creating jobs.`,
        "error",
      );
      return;
    }
    ...
    <div className="hbl-app">
      <div className="topbar">
        ...
        <div className="search-wrap">
          ...
          <input
            placeholder="Search TXN ID, filename, BL number…"
            value={search}
        ...
        <div className="top-actions">
          <div className="top-stats">
            ...
            <span>
              Done{" "}
              <strong style={{ color: "var(--green)" }}>{doneCount}</strong>
            </span>
            <span>
              Failed{" "}
              <strong style={{ color: "var(--red)" }}>{failedCount}</strong>
            </span>
          </div>
          <button className="btn btn-ghost" onClick={loadFiles} title="Refresh">
            ↻
          </button>
          {selected.size > 0 && (
            <button
              ...
              title={
                selectedHasValidationError
                  ? "Fix invalid port/carrier/container type codes before creating jobs"
                  : undefined
              }
              style={
                selectedHasValidationError
            ...
            >
              {jobLoading && (
                <span
                  style={{
                    ...
                    borderTopColor: "#fff",
                    borderRadius: "50%",
                    animation: "hbl-spin .7s linear infinite",
              ...
              {selectedHasValidationError && (
                <span style={{ fontSize: ".65rem", marginLeft: 4 }}>⚠</span>
              )}
            </button>
          )}
          <button
            ...
            <tr>
              <th>
                <input
                  ...
                  checked={
                    pageRows.length > 0 &&
                    pageRows.every((f) => selected.has(f.txn_id))
                  }
              ...
              <th>PDF Type</th>
              <th onClick={() => sortToggle("uploaded_at")}>
                Uploaded{" "}
                {sortKey === "uploaded_at" ? (sortDir === 1 ? "↑" : "↓") : "↓"}
              </th>
              <th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {loadState !== "loading" &&
              pageRows.map((f) => (
                <tr
                  key={f.txn_id}
                  className={selected.has(f.txn_id) ? "selected" : ""}
                  onClick={() => setModal({ type: "detail", record: f })}
                >
                  <td onClick={(e) => e.stopPropagation()}>
                    <input
                      type="checkbox"
                      checked={selected.has(f.txn_id)}
                      onChange={(e) => toggleRow(f.txn_id, e.target.checked)}
                    />
                  </td>
                  <td className="td-txn" title={f.txn_id || undefined}>
                    {f.txn_id}
                  </td>
                  <td className="td-file">
                    <div className="td-filename" title={f.filename || undefined}>
                    ...
                    <div
                      className="td-file-meta"
                      title={
                        f.size_kb != null ? `${f.size_kb} KB` : undefined
                      }
                    >
                      {f.size_kb} KB
                    </div>
                  </td>
                  <td className="td-bl" title={f.bl_number || undefined}>
                    {f.bl_number ?? "—"}
                  </td>
                  <td className="td-status">
                    <Badge status={f.status} />
                  </td>
                  <td className="td-user" title={f.created_by || undefined}>
                    {f.created_by ?? "—"}
                  </td>
                  <td className="td-pdf">
                    <span
                      className="pdf-tag"
                      title={f.pdf_type || undefined}
                    >
                      {f.pdf_type ?? "—"}
                    </span>
                  </td>
                  <td
                    className="td-date"
                    title={f.uploaded_at || undefined}
                  >
                    {f.uploaded_at ? f.uploaded_at.slice(0, 16) : "—"}
                  </td>
                  <td onClick={(e) => e.stopPropagation()}>
                    <div className="action-row">
                      ...
                      >
                        ...
                        <span style={{ marginLeft: "2px", marginTop: "2px", fontSize: "12px" }}>EDIT</span>
                      </button>
                      {hasViewableFiles(f) && (
                        <button
                          className="act-btn dl"
                        ...
                        >
                          <span style={{ marginLeft: "2px", marginTop: "2px", fontSize: "12px" }}>↓ PDF</span>
                        </button>
                      )}
                    </div>
                  </td>
      ...
      <div className="pagination">
        <div className="pag-info">
          Showing <strong>{filtered.length ? pageStart + 1 : 0}</strong>–
            ...
            <>
              ...
              {buildPageRange(page, totalPages).map((p_, i) =>
                p_ === "..." ? (
                  ...
                  <button
                    key={i}
                    className={`pag-btn ${page === p_ ? "active" : ""}`}
      ...
      <div className={`action-bar ${selected.size > 0 ? "visible" : ""}`}>
        ...
        <button className="bar-btn danger" onClick={deleteSelected}>
          🗑 Delete
        </button>
        <button className="bar-btn outline" onClick={clearSel}>
          ✕ Clear
        </button>
      </div>
      ...
      {modal?.type === "files" && (
        <FilesListModal
          record={modal.record}
          onClose={() => setModal(null)}
          onSelectFile={(file, sectionTitle) =>
            setModal({
              type: "filePreview",
      ...
      {modal?.type === "filePreview" && (
        <FilePreviewModal
          ...
          onBack={
            modal.backTo
              ? () => setModal({ type: "files", record: modal.backTo! })
              : undefined
          }
        />
      )}
