...
function defaultEnquiryBody(row: EnquiryRow): string {
  const sp =
    row.salespersonName?.trim() ||
    row.drilldownEnquiry.sales_person?.trim() ||
    "";
  const lines = [
    `Enquiry: ${row.enquiryCode}`,
    `Customer: ${row.customer}`,
    sp ? `Salesperson: ${sp}` : null,
    `Lane: ${row.lane}`,
    `Mode: ${row.modeLabel}`,
...
export function TopActiveEnquirySendEmailModal({ opened, onClose, row }: Props) {
  ...
  useEffect(() => {
    ...
    const subject = `Enquiry Conversion - ${row.customer.trim()} · ${row.enquiryCode}`;
    setEmailForm({
      to_email: cleanedTo,
      cc_email: cleanedCc,
      subject,
      message: defaultEnquiryBody(row),
    });
  ...
  const handleSendEmail = async () => {
    if (!row) {
      toast.error("No enquiry data available to send email");
      return;
    }
