...
function EnquiryCreate() {
  ...
  const handleNext = () => {
    ...
    if (active === 1) {
      // Direct quote flows: validate service details, then go to quotation step
      if (
        (enq?.fromDestination || enq?.fromQuotationList) &&
        enq?.actionType === "createQuote"
      ) {
        const serviceResult = serviceForm.validate();
        if (serviceResult.hasErrors) return;
        setActive(2);
        return;
      }
  ...
  useEffect(() => {
    try {
      ...
      if (enq !== null) {
        ...
        // Check if this is from destination or quotation-list direct create flow
        if (
          enq.actionType === "createQuote" &&
          (enq.fromDestination || enq.fromQuotationList)
        ) {
