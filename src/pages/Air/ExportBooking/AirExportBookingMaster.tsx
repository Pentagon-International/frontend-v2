...
function AirExportBookingMaster() {
  ...
  const handleCreateJob = async (booking: ExportShipmentData) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      agent_code: booking.destination_agent_code || "",
      agent_name: booking.destination_agent_name || "",
      origin_code: booking.origin_code_read || booking.origin_code || "",
      origin_name: booking.origin_name || "",
      destination_code: booking.destination_code_read || booking.destination_code || "",
      ...
      carrier_code: booking.carrier_code_read || "",
      flightno: booking.voyage_no || "",
      is_direct: (booking as ExportShipmentData & { is_direct?: boolean }).is_direct || false,
      mawb_no: booking.mawb_no || "",
      mbl_date: booking.mawb_date ? dayjs(booking.mawb_date).isValid() ? dayjs(booking.mawb_date).format("YYYY-MM-DD") : null : null,
      ...
      housing_details: housingDetails.length > 0 ? housingDetails.map((h) => ({
        hawb_no: (h.hawb_no as string) || (h.hawb_number as string) || (h.hbl_number as string) || "",
        ...
        origin_code: (h.origin_code as string) || booking.origin_code_read || booking.origin_code || "",
        origin_name: (h.origin_name as string) || booking.origin_name || "",
        destination_code: (h.destination_code as string) || booking.destination_code_read || booking.destination_code || "",
        destination_name: (h.destination_name as string) || booking.destination_name || "",
        trade: "Re Export",
        ...
        agent_code: (h.agent_code as string) || booking.destination_agent_code || "",
        agent_name: (h.agent_name as string) || booking.destination_agent_name || "",
        agent_address: (h.agent_address as string) || booking.destination_agent_address || "",
        agent_email: (h.agent_email as string) || booking.destination_agent_email || "",
        shipper_name: (h.shipper_name as string) || booking.shipper_name || "",
        shipper_address: (h.shipper_address as string) || booking.shipper_address || "",
        shipper_email: (h.shipper_email as string) || booking.shipper_email || "",
        consignee_name: (h.consignee_name as string) || booking.consignee_name || "",
        consignee_address: (h.consignee_address as string) || booking.consignee_address || "",
        consignee_email: (h.consignee_email as string) || booking.consignee_email || "",
        notify1_customer_name: (h.notify1_customer_name as string) || booking.notify1_customer_name || "",
        notify1_customer_address: (h.notify1_customer_address as string) || booking.notify1_customer_address || "",
        notify1_customer_email: (h.notify1_customer_email as string) || booking.notify1_customer_email || "",
        notify2_customer_name: (h.notify2_customer_name as string) || "",
        notify2_customer_address: (h.notify2_customer_address as string) || "",
        notify2_customer_email: (h.notify2_customer_email as string) || "",
        cha_name: (h.cha_name as string) || booking.cha || "",
        cha_address: (h.cha_address as string) || booking.cha_address || "",
        ...
        shipment_terms_code: (h.shipment_terms_code as string) || booking.shipment_terms_code || "",
        events: Array.isArray(h.events) ? (h.events as Array<Record<string, unknown>>).map((event) => ({
        ...
        })) : Array.isArray(booking.events) ? (booking.events as Array<Record<string, unknown>>).map((event) => ({
          event_id: event.event_id || "",
          event_name: event.event_name || "",
          event_date: event.event_date || "",
          event_status: event.event_status || "",
        ...
        cargo_details: Array.isArray(h.cargo_details) ? (h.cargo_details as Array<Record<string, unknown>>).map((cargo) => ({
          ...
          gross_weight: Number((cargo.gross_weight as number)) || 0,
        ...
        mawb_charges: Array.isArray(h.mawb_charges) ? (h.mawb_charges as Array<Record<string, unknown>>).map((charge) => ({
          charge_id: Number(charge.charge_id) || 0,
          ...
          charge_id: Number((charge.charge_id as number)) || 0,
          supplier_code: "",
          pp_cc: String((charge.pp_cc as string) || "Prepaid"),
          unit_id: Number((charge.unit_id as number)) || 0,
          no_of_unit: Number((charge.no_of_unit as number) ?? (charge.no_of_units as number) ?? 0),
          amount: Number((charge.min_sell as number) ?? (charge.sell_amount_total as number)) || 0,
          amount_per_unit: Number((charge.sell_per_unit as number)) || 0,
          cost_local_amount: Number((charge.cost_local_amount as number)) || 0,
          currency_id: Number((charge.currency_id as number)) || 0,
          roe: Number((charge.roe as number)) || 1,
          sell_local_amount: Number((charge.sell_local_amount as number)) || 0,
        ...
        notify1_customer_address: booking.notify_customer_address || "",
        notify1_customer_email: booking.notify_customer_email || "",
        ...
        events: Array.isArray(booking.events) ? (booking.events as Array<Record<string, unknown>>).map((event) => ({
          event_id: event.event_id || "",
          event_name: event.event_name || "",
          event_date: event.event_date || "",
          event_status: event.event_status || "",
