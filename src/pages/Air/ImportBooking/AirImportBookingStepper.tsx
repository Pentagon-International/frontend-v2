...
const validationSchema = yup.object({
  ...
  forwarder_email: yup
    ...
    .nullable()
    .notRequired(),
  destination_agent_code: yup.string().required("Origin agent is required"),
  ...
  destination_agent_email: yup
    .string()
    .email("Invalid email format")
    .required("Origin agent email is required"),
...
const AirImportBookingStepper: React.FC<ImportShipmentStepperProps> = ({
  onStepChange,
  onComplete,
  initialData,
  isEditMode = false,
  ...
  const shipmentOptions = useMemo(() => {
    ...
    const options = termsOfShipment.map((item: TermsOfShipmentData) => ({
    ...
    if (isEditMode && initialData?.shipment_terms_code) {
      const shipmentTermsCodeValue = String(initialData.shipment_terms_code);
      const shipmentTermsName = String(initialData.shipment_terms_name || "");
      const exists = options.some(
        (opt) => opt.value === shipmentTermsCodeValue,
      );
      if (!exists && shipmentTermsCodeValue) {
        options.unshift({
          value: shipmentTermsCodeValue,
          label: shipmentTermsName
            ? `${shipmentTermsName} (${shipmentTermsCodeValue})`
            : shipmentTermsCodeValue,
        });
      }
    }
  ...
  const mapInitialDataToFormValues = (
    ...
    return {
      ...
      notify2_customer_code: String(
        data.notify2_customer_code_read || data.notify2_customer_code || "",
      ),
      ...
      planned_pickup_date: data.planned_pickup_date
        ...
        : new Date(),
      actual_pickup_date:
        data.actual_pickup_date && String(data.actual_pickup_date) !== "null"
  ...
  const handleFreightCertificateDownloadPDF = () => {
    if (freightCertificatePdfBlob) {
      ...
      link.download = `FreightCertificate-${jobData?.shipment_code ?? "shipment"}.pdf`;
  ...
  const salespersonsData = useMemo(() => {
    ...
    if (
      ...
      response.data.forEach((item: SalespersonData) => {
        options.push({
  ...
  useEffect(() => {
    ...
    if (initialData.consignee_address) {
      setConsigneeAddressOptions([
        {
          value: String(initialData.consignee_address),
          label: String(initialData.consignee_address),
          id: initialData.consignee_address_id,
        },
      ]);
      form.setFieldValue(
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Navigate back to air import booking master page with refresh flag
  ...
  return (
    <>
      {/* Events modal - single heading row, multiple data rows (like Ocean export) */}
      <Modal
              ...
              <Grid mb="md">
                ...
                <Grid.Col span={12}>
                  <PartyAddressField
                    ...
                    partyKey={form.values.notify1_customer_name || ""}
                    onChange={(next, option) => {
              ...
              <Grid mb="md">
                ...
                <Grid.Col span={12}>
                  <PartyAddressField
                    ...
                    partyKey={form.values.notify2_customer_name || ""}
                    onChange={(next, option) => {
...
export default AirImportBookingStepper;
