...
/** `summary` on `reversePaymentFilter` — totals are filter-scoped. */
type PaymentReversalListSummary = {
...
type PaymentReversalListQueryResult = {
  data: PaymentReversalRow[];
  summary?: PaymentReversalListSummary;
};
...
type PaymentReversalColumnVisibility = {
  sno: boolean;
...
const paymentReversalColumnDefault: PaymentReversalColumnVisibility = {
  sno: true,
  day_book_name: true,
...
const paymentReversalColumnLabels: Record<keyof PaymentReversalColumnVisibility, string> = {
  sno: "S.No",
...
function paymentReversalColumnId<T extends Record<string, unknown>>(
  col: MRT_ColumnDef<T>,
): string {
...
export default function PaymentReversalMaster() {
  ...
  const DEFAULT_FILTERS: PaymentReversalFilters = {
    day_book_id: "",
    day_book_name: "",
    reverse_payment_no: "",
    original_doc_no: "",
  ...
  const [draftFilters, setDraftFilters] =
    useState<PaymentReversalFilters>(DEFAULT_FILTERS);
  ...
  const [visibleColumns, setVisibleColumns] = useState<PaymentReversalColumnVisibility>(
    () => ({ ...paymentReversalColumnDefault }),
  );
  ...
  const commitHeaderFilters = useCallback(
    (updater: (prev: PaymentReversalFilters) => PaymentReversalFilters) => {
  ...
  const {
    data: paymentReversalListResult,
    isLoading: listLoading,
    isFetching: listFetching,
    error: listError,
  } = useQuery<PaymentReversalListQueryResult>({
    queryKey: [
      "payment-reversal",
    ...
    queryFn: async (): Promise<PaymentReversalListQueryResult> => {
      try {
        ...
        const response = (await apiCallProtected.post(
          `${URL.reversePaymentFilter}?index=${index}&limit=${pagination.pageSize}`,
          payload,
        )) as Record<string, unknown>;

        const raw = response as any;
        const bodyCandidate = raw?.data != null && !Array.isArray(raw.data) ? raw.data : raw;
        const body = bodyCandidate != null ? (bodyCandidate as PaymentReversalFilterResponse | PaymentReversalRow[]) : null;
        if (!body) {
          setTotalRecords(0);
          return { data: [], summary: undefined };
        }
        const list = Array.isArray((body as PaymentReversalFilterResponse).data)
          ? ((body as PaymentReversalFilterResponse).data as PaymentReversalRow[])
          : Array.isArray(body) ? (body as PaymentReversalRow[]) : [];
        const totalEnvelope =
          body != null &&
          typeof body === "object" &&
          !Array.isArray(body) &&
        ...
        const summary: PaymentReversalListSummary | undefined =
          rawSummary && typeof rawSummary === "object" && !Array.isArray(rawSummary)
            ? (rawSummary as PaymentReversalListSummary)
            : undefined;
  ...
  const listData = paymentReversalListResult?.data ?? [];
  ...
  const listStats = useMemo(() => {
    ...
    const summary = paymentReversalListResult?.summary;
    if (summary) {
  ...
  }, [tableData, paymentReversalListResult?.summary, totalRecords]);

  const filterFieldStyles = erpListFilterUnifiedMantineStyles(erpTheme);

  const columnToggleItems = useMemo(
    () =>
      (Object.keys(visibleColumns) as (keyof PaymentReversalColumnVisibility)[]).map(
        (key) => ({
          id: String(key),
          label: paymentReversalColumnLabels[key],
  ...
  const allColumns = useMemo<MRT_ColumnDef<PaymentReversalRow>[]>(
    () => [
      {
        id: "sno",
        header: "S.No",
      ...
      },
      {
        ...
        Header: () => (
          <ERPListColumnHeaderFilter
            label="Reverse Payment No"
            value={appliedFilters.reverse_payment_no}
            displayValue={appliedFilters.reverse_payment_no}
            onChange={() => {}}
            theme={erpTheme}
            isEditing={editingHeaderId === "reverse_payment_no"}
            onStartEdit={() => openHeaderEditor("reverse_payment_no")}
            onStopEdit={() => collapseHeaderEditor("reverse_payment_no")}
            renderEditor={({ autoFocus, onClose }) => (
              <SearchableSelect
                apiEndpoint={URL.reversePayment}
                placeholder="Reverse Payment No"
                value={appliedFilters.reverse_payment_no}
                onChange={(val) => {
                  commitHeaderFilters((prev) => ({
                    ...prev,
                    reverse_payment_no: val || "",
                  }));
                  if (val) onClose();
                }}
                dropdownZIndex={1000}
                minSearchLength={1}
                displayFormat={(item) => ({
                  value: String(item.reverse_payment_no ?? item.payment_no ?? ""),
                  label: String(item.reverse_payment_no ?? item.payment_no ?? ""),
                })}
  ...
  const columns = useMemo(
    () =>
      allColumns.filter((col) => {
        const id = paymentReversalColumnId(col);
        if (id === "actions") return true;
        return visibleColumns[id as keyof PaymentReversalColumnVisibility] !== false;
      }),
    ...
    renderEmptyRowsFallback: () => (
      ...
      >
        {isLoading ? (
          ...
          <Text c="dimmed" size="sm" style={{ fontFamily: erpTheme.fontSans }}>
            No payment reversal data found
          </Text>
        )}
      </Center>
    ),
      ...
      >
        <ERPListScreen
          ...
          filters={{
            opened: showFilters,
            title: "Filters",
            subtitle: "Refine by day book, reverse payment number, dates, type, or status",
                ...
                <Grid.Col span={ERP_LIST_FILTER_FIELD_COL_SPAN}>
                  <Box style={erpListFilterFieldCellStyle}>
                    <SearchableSelect
                      apiEndpoint={URL.reversePayment}
                      label="Reverse Payment No"
                      placeholder="Type Reverse Payment No"
                      value={draftFilters.reverse_payment_no}
                      onChange={(val) => setDraftFilters((prev) => ({ ...prev, reverse_payment_no: val || "" }))}
                      dropdownZIndex={1000}
                      minSearchLength={1}
                      displayFormat={(item) => ({
                        value: String(item.reverse_payment_no ?? item.payment_no ?? ""),
