...
/** `summary` on `reverseSupplierInvoiceFilter` — totals are filter-scoped. */
type SupplierInvoiceReversalListSummary = {
...
type SupplierInvoiceReversalListQueryResult = {
  data: SupplierInvoiceReversalRow[];
  summary?: SupplierInvoiceReversalListSummary;
};
...
const supplierInvoiceReversalColumnLabels: Record<
  keyof SupplierInvoiceReversalColumnVisibility,
  string
> = {
  sno: "S.No",
...
function supplierInvoiceReversalColumnId<T extends Record<string, unknown>>(
  col: MRT_ColumnDef<T>,
): string {
...
function SupplierInvoiceReversalMaster() {
  ...
  const [visibleColumns, setVisibleColumns] =
    useState<SupplierInvoiceReversalColumnVisibility>(() => ({
      ...supplierInvoiceReversalColumnDefault,
    }));
  ...
  const commitHeaderFilters = useCallback(
    (
      updater: (
        prev: SupplierInvoiceReversalFilters,
      ) => SupplierInvoiceReversalFilters,
    ) => {
  ...
  const {
    data: supplierInvoiceReversalListResult,
    isLoading: reversalLoading,
    isFetching: reversalFetching,
    error: reversalError,
  } = useQuery<SupplierInvoiceReversalListQueryResult>({
    queryKey: [
    ...
    queryFn: async (): Promise<SupplierInvoiceReversalListQueryResult> => {
      try {
        ...
        const body = bodyCandidate != null
          ? (bodyCandidate as SupplierInvoiceReversalFilterResponse | SupplierInvoiceReversalRow[])
          : null;
        ...
        const list = Array.isArray((body as SupplierInvoiceReversalFilterResponse).data)
          ? ((body as SupplierInvoiceReversalFilterResponse).data as SupplierInvoiceReversalRow[])
          : Array.isArray(body) ? (body as SupplierInvoiceReversalRow[]) : [];
        ...
        const summary: SupplierInvoiceReversalListSummary | undefined =
          rawSummary && typeof rawSummary === "object" && !Array.isArray(rawSummary)
            ? (rawSummary as SupplierInvoiceReversalListSummary)
            : undefined;
  ...
  const reversalData = supplierInvoiceReversalListResult?.data ?? [];
  ...
  const listStats = useMemo(() => {
    ...
    const summary = supplierInvoiceReversalListResult?.summary;
    if (summary) {
  ...
  }, [tableData, supplierInvoiceReversalListResult?.summary, totalRecords]);
  ...
  const columnToggleItems = useMemo(
    () =>
      (Object.keys(visibleColumns) as (keyof SupplierInvoiceReversalColumnVisibility)[]).map(
        (key) => ({
          id: String(key),
          label: supplierInvoiceReversalColumnLabels[key],
  ...
  const allColumns = useMemo<MRT_ColumnDef<SupplierInvoiceReversalRow>[]>(
    () => [
      {
        id: "sno",
        header: "S.No",
  ...
  const columns = useMemo(
    () =>
      allColumns.filter((col) => {
        const id = supplierInvoiceReversalColumnId(col);
        if (id === "actions") return true;
        return visibleColumns[id as keyof SupplierInvoiceReversalColumnVisibility] !== false;
      }),
    ...
    renderEmptyRowsFallback: () => (
      ...
      >
        {isLoading ? (
          ...
          <Text c="dimmed" size="sm" style={{ fontFamily: erpTheme.fontSans }}>
            No supplier invoice reversal data found
          </Text>
        )}
      </Center>
    ),
