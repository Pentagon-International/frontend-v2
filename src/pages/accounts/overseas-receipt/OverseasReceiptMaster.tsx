...
type OverseasReceiptListQueryResult = {
  data: ReceiptRow[];
  summary?: OverseasReceiptListSummary;
};
...
function overseasReceiptColumnId(
...
export default function OverseasReceiptMaster() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<OverseasReceiptColumnVisibility>(
    () => ({ ...overseasReceiptColumnDefault }),
  );
  ...
  const commitHeaderFilters = useCallback(
    (updater: (prev: OverseasReceiptFilters) => OverseasReceiptFilters) => {
  ...
  const {
    ...
    error: receiptError,
  } = useQuery<OverseasReceiptListQueryResult>({
    queryKey: [
      "overseas-receipt",
    ...
    queryFn: async (): Promise<OverseasReceiptListQueryResult> => {
      try {
        ...
        const summary: OverseasReceiptListSummary | undefined =
          rawSummary && typeof rawSummary === "object" && !Array.isArray(rawSummary)
            ? (rawSummary as OverseasReceiptListSummary)
            : undefined;
  ...
  const columnToggleItems = useMemo(
    () =>
      (Object.keys(visibleColumns) as (keyof OverseasReceiptColumnVisibility)[]).map(
        (key) => ({
          id: String(key),
          label: overseasReceiptColumnLabels[key],
  ...
  const columns = useMemo(
    () =>
      allColumns.filter((col) => {
        const id = overseasReceiptColumnId(col);
        if (id === "actions") return true;
        return visibleColumns[id as keyof OverseasReceiptColumnVisibility] !== false;
      }),
        ...
        {isLoading ? (
          <Stack align="center" gap="md">
            ...
            <Text c="dimmed" size="sm" style={{ fontFamily: erpTheme.fontSans }}>
              Loading overseas receipt data…
            </Text>
          </Stack>
        ) : (
          <Text c="dimmed" size="sm" style={{ fontFamily: erpTheme.fontSans }}>
            No overseas receipt data found
          </Text>
        )}
      </Center>
    ),
      ...
      >
        <ERPListScreen
          ...
          filters={{
            opened: showFilters,
            title: "Filters",
            subtitle: "Overseas (agent) receipts — refine by day book, receipt no., dates, type, or status",
            ...
            children: receiptError ? (
              <Center py="xl" style={{ backgroundColor: cardBg, flex: 1, minHeight: 200 }}>
                <Text size="sm" c="dimmed" style={{ fontFamily: erpTheme.fontSans }}>
                  Error loading overseas receipt data. Please try refreshing the page.
                </Text>
              </Center>
            ) : (
              /*
