...
function partyHeaderNetSign(drCr: "Cr" | "Dr", isReversal: boolean): 1 | -1 {
  const isDr = drCr === "Dr";
...
export default function OverseasReceiptCreate({
  titleOverride = "Create Overseas Receipt",
  backPath = "/overseas-receipt",
  ...
  const { data: currencyData = [] } = useQuery({
    queryKey: ["currencyMaster", "overseas-receipt"],
  ...
  const { data: daybookDataRPT = [] } = useQuery({
    queryKey: ["daybook", "RPT", "overseas-receipt"],
    queryFn: fetchDaybookRPT,
    staleTime: Infinity,
  });
  const { data: daybookDataRPTREV = [] } = useQuery({
    queryKey: ["daybook", "RPTREV", "overseas-receipt"],
  ...
  const { data: daybookDataForAdjustments = [] } = useQuery({
    queryKey: ["daybook", "INV", "overseas-receipt"],
  ...
  useEffect(() => {
    ...
    const localAmountVal = toLocalAmount(receiptFromState.local_amount);
    ...
    const details: DetailRow[] =
      parties.length > 0
        ? parties.map((p) => ({
            ...
            account_code: String(
              (p as { account_code?: string }).account_code ?? "",
            ).trim(),
    ...
    if (_isReversal) {
      ...
      setSaveResponse({
        ...
        status: (receiptFromState.status ?? "UNPOSTED").toString(),
        jv_no:
          receiptFromState.jv_no != null &&
          String(receiptFromState.jv_no).trim()
            ? String(receiptFromState.jv_no).trim()
            : null,
      });
    }
  ...
  const pageTitle = pathname.includes("/reversal/view")
    ...
    : pathname.includes("/reversal/edit")
      ...
      : pathname.includes("/reversal/create")
        ...
        : pathname.includes("/overseas-receipt/view") ||
            pathname.includes("/receipt/view")
          ? "View Overseas Receipt"
          : pathname.includes("/overseas-receipt/edit") ||
              pathname.includes("/receipt/edit")
            ? "Edit Overseas Receipt"
            : pathname.includes("/overseas-receipt/create") ||
                pathname.includes("/receipt/create")
              ? "Create Overseas Receipt"
              : titleOverride;
