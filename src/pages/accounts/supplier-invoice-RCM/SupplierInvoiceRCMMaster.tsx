...
export default function SupplierInvoiceMaster() {
  ...
  const {
    ...
    queryFn: async (): Promise<SupplierInvoiceRow[]> => {
      try {
        ...
        setTotalRecords(Number(body?.total ?? list.length));
        return list;
      } catch (err: unknown) {
  ...
  const columns = useMemo<MRT_ColumnDef<SupplierInvoiceRow>[]>(
    () => [
      {
        id: "sno",
        header: "S.No",
    ...
    >
      ...
      {isLoading ? (
        ...
        <Center py="xl" style={{ flex: 1 }}>
          <Stack align="center" gap="md">
            <Text c="dimmed">
              Error loading supplier invoice data. Please try refreshing the
              page.
            </Text>
          </Stack>
        </Center>
