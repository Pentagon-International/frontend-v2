...
/** `summary` on `supplierInvoiceFilter` with `is_rcm: true` — totals are filter-scoped. */
type SupplierInvoiceRCMListSummary = {
...
type SupplierInvoiceRCMListQueryResult = {
  data: SupplierInvoiceRow[];
  summary?: SupplierInvoiceRCMListSummary;
};
...
const LIST_KEY = "SUPPLIER_INVOICE_RCM_MASTER";

type SupplierInvoiceRCMFilters = {
  invoice_no: string;
  agent_name: string;
  date_from: Date | null;
  date_to: Date | null;
...
type SupplierInvoiceRCMColumnVisibility = {
  sno: boolean;
  invoice_no: boolean;
  agent_name: boolean;
  date: boolean;
...
const rcmColumnDefault: SupplierInvoiceRCMColumnVisibility = {
  sno: true,
  invoice_no: true,
  agent_name: true,
  date: true,
...
const rcmColumnLabels: Record<keyof SupplierInvoiceRCMColumnVisibility, string> = {
  sno: "S.No",
...
function rcmColumnId<T extends Record<string, unknown>>(
  col: MRT_ColumnDef<T>,
): string {
...
function SupplierInvoiceRCMMaster() {
  ...
  const DEFAULT_FILTERS: SupplierInvoiceRCMFilters = {
    invoice_no: "",
    agent_name: "",
  ...
  const [visibleColumns, setVisibleColumns] = useState<SupplierInvoiceRCMColumnVisibility>(
    () => ({ ...rcmColumnDefault }),
  );
  ...
  useEffect(() => {
    ...
    if (stored?.filters && typeof stored.filters === "object") {
      ...
      const restored: SupplierInvoiceRCMFilters = {
        ...DEFAULT_FILTERS,
        ...raw,
  ...
  const buildFiltersPayload = (
    filters: SupplierInvoiceRCMFilters,
    searchValue: string,
  ) => {
  ...
  const {
    data: supplierInvoiceRcmListResult,
    isLoading: rcmLoading,
    isFetching: rcmFetching,
    error: rcmError,
  } = useQuery<SupplierInvoiceRCMListQueryResult>({
    queryKey: [
    ...
    queryFn: async (): Promise<SupplierInvoiceRCMListQueryResult> => {
      try {
        ...
        const summary: SupplierInvoiceRCMListSummary | undefined =
          rawSummary && typeof rawSummary === "object" && !Array.isArray(rawSummary)
            ? (rawSummary as SupplierInvoiceRCMListSummary)
            : undefined;
  ...
  const rcmData = supplierInvoiceRcmListResult?.data ?? [];
  ...
  const isLoading = rcmFetching || rcmLoading || isInitialLoad;
  const tableData = rcmData ?? [];
  ...
  const listStats = useMemo(() => {
    ...
    const summary = supplierInvoiceRcmListResult?.summary;
    if (summary) {
  ...
  }, [tableData, supplierInvoiceRcmListResult?.summary, totalRecords]);
  ...
  const columnToggleItems = useMemo(
    () =>
      (Object.keys(visibleColumns) as (keyof SupplierInvoiceRCMColumnVisibility)[]).map(
        (key) => ({
          id: String(key),
          label: rcmColumnLabels[key],
  ...
  const columns = useMemo(
    () =>
      allColumns.filter((col) => {
        const id = rcmColumnId(col);
        if (id === "actions") return true;
        return visibleColumns[id as keyof SupplierInvoiceRCMColumnVisibility] !== false;
      }),
              ...
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setStoreSearch(LIST_KEY, search);
                    setShouldRestore(LIST_KEY, true);
                    navigate("/supplier-invoice-rcm/create");
                  }}
                >
                  Create New
                </Button>
            ...
            children: rcmError ? (
              <Center py="xl" style={{ backgroundColor: cardBg, flex: 1, minHeight: 200 }}>
                <Text size="sm" c="dimmed" style={{ fontFamily: erpTheme.fontSans }}>
                  Error loading supplier invoice (RCM) data. Please try refreshing the page.
                </Text>
              </Center>
            ) : isLoading ? (
              <ERPListTableLoading theme={erpTheme} message="Loading supplier invoice (RCM) data…" />
            ) : tableData.length === 0 ? (
              <ERPListTableEmpty
                theme={erpTheme}
                icon={<IconCreditCard size={24} color={muted} />}
                title="No RCM supplier invoices found"
                hint="Try clearing filters or adjust your search"
              />
            ) : (
...
export default SupplierInvoiceRCMMaster;
