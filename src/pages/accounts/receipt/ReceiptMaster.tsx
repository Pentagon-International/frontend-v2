...
/** `summary` on `receiptFilter` (totals are filter-scoped). */
type ReceiptListSummary = {
...
type ReceiptMasterColumnVisibility = {
  sno: boolean;
...
const receiptMasterColumnDefault: ReceiptMasterColumnVisibility = {
  sno: true,
  day_book_name: true,
  receipt_no: true,
  date: true,
...
const receiptMasterColumnLabels: Record<keyof ReceiptMasterColumnVisibility, string> = {
  sno: "S.No",
...
function receiptColumnId<T extends Record<string, unknown>>(
  col: MRT_ColumnDef<T>,
): string {
...
export default function ReceiptMaster() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<ReceiptMasterColumnVisibility>(
    () => ({ ...receiptMasterColumnDefault }),
  );
  ...
  const commitHeaderFilters = useCallback(
    (updater: (prev: ReceiptFilters) => ReceiptFilters) => {
  ...
  const {
    ...
    error: receiptError,
  } = useQuery<ReceiptListQueryResult>({
    queryKey: [
      "receipt",
    ...
    queryFn: async (): Promise<ReceiptListQueryResult> => {
      try {
        ...
        const summary: ReceiptListSummary | undefined =
          rawSummary && typeof rawSummary === "object" && !Array.isArray(rawSummary)
            ? (rawSummary as ReceiptListSummary)
            : undefined;
  ...
  const columnToggleItems = useMemo(
    () =>
      (Object.keys(visibleColumns) as (keyof ReceiptMasterColumnVisibility)[]).map(
        (key) => ({
          id: String(key),
          label: receiptMasterColumnLabels[key],
      ...
      },
      {
        ...
        Header: () => (
          <ERPListColumnHeaderFilter
            ...
            onChange={() => {}}
            theme={erpTheme}
            isEditing={editingHeaderId === "receipt_no"}
            onStartEdit={() => openHeaderEditor("receipt_no")}
            onStopEdit={() => collapseHeaderEditor("receipt_no")}
            renderEditor={({ autoFocus, onClose }) => (
              <SearchableSelect
  ...
  const columns = useMemo(
    () =>
      allColumns.filter((col) => {
        const id = receiptColumnId(col);
        if (id === "actions") return true;
        return visibleColumns[id as keyof ReceiptMasterColumnVisibility] !== false;
      }),
  ...
  const table = useMantineReactTable({
    ...
    mantineTableBodyCellProps: ({ column }) => {
      ...
      const extraStyles = isActions
        ? {
            // Pinned-right Actions cell. Matches the head cell's
            // `minWidth: 80px` so the sticky body cell and sticky head cell
            // are exactly the same width (no visual gap at the corner).
            // `zIndex: 2` lets sticky body cells layer above non-sticky
            // body cells while staying BELOW the sticky head cell
            // (`zIndex: 4`).
    ...
    mantineTableHeadCellProps: ({ column }) => {
      ...
      return {
        style: {
           ...
           * and the inline filter editor (Day Book / Receipt No / Type /
           * Status) never resizes the row. 52px matches EnquiryMaster's
           * header row height (height: 52.4) and accommodates a 32px input.
           */
          minHeight: 52,
          height: 52,
      ...
      >
        <ERPListScreen
          ...
          filters={{
            opened: showFilters,
            title: "Filters",
            subtitle: "Refine by day book, receipt number, dates, type, or status",
