...
function JobClosure() {
  ...
  const [closeConfirmRow, setCloseConfirmRow] = useState<ImportJobData | null>(
    null
  );
  ...
  useEffect(() => {
    if (location.state?.refreshData) {
      queryClient.invalidateQueries({ queryKey: ["jobClosure"] });
      refetchJobClosure();
    }
  }, [location.state?.refreshData, queryClient, refetchJobClosure]);

  const handleConfirmClose = async () => {
    if (!closeConfirmRow) return;
    const rowToClose = closeConfirmRow;
    setIsClosing(true);
    try {
      await closeJob(rowToClose.id);
      setCloseConfirmRow(null);
      ToastNotification({
        type: "success",
        message: "Job closed successfully",
      });
      await refetchJobClosure();
    } catch (err: unknown) {
      ToastNotification({
        type: "error",
  ...
  return (
    <MantineProvider theme={erpListGeistMantineTheme}>
      <Box className={ERP_LIST_GEIST_ROOT_CLASS} style={erpListGeistRootTypography}>
                          ...
                          >
                            ...
                            <td style={erpListStickyActionTdStyle(theme)}>
                              <ERPListJobActionMenu
                                status={row.status}
                                variant="closure-page"
                                onCloseJob={() => setCloseConfirmRow(row)}
                              />
                            </td>
                          </tr>
                        );
        ...
        <Modal
          opened={!!closeConfirmRow}
          onClose={() => !isClosing && setCloseConfirmRow(null)}
          title={
        ...
        >
          ...
          <Group justify="flex-end" gap="xs">
            <Button
              variant="subtle"
              onClick={() => setCloseConfirmRow(null)}
              disabled={isClosing}
            >
              No
            </Button>
            <Button color="blue" onClick={handleConfirmClose} loading={isClosing}>
              Yes, close
            </Button>
          </Group>
        </Modal>
