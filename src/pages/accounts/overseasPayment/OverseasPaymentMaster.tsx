...
const LIST_KEY = "OVERSEAS_PAYMENT_MASTER";
...
type OverseasPaymentListQueryResult = {
  data: PaymentRow[];
  summary?: OverseasPaymentListSummary;
};
...
function overseasPaymentColumnId<T extends Record<string, unknown>>(
  col: MRT_ColumnDef<T>,
): string {
...
export default function OverseasPaymentMaster() {
  ...
  const DEFAULT_FILTERS: OverseasPaymentFilters = {
    day_book_id: "",
    day_book_name: "",
    payment_no: "",
  ...
  const [draftFilters, setDraftFilters] =
    useState<OverseasPaymentFilters>(DEFAULT_FILTERS);
  ...
  const [visibleColumns, setVisibleColumns] = useState<OverseasPaymentColumnVisibility>(
    () => ({ ...overseasPaymentColumnDefault }),
  );
  ...
  const commitHeaderFilters = useCallback(
    (updater: (prev: OverseasPaymentFilters) => OverseasPaymentFilters) => {
  ...
  const buildFiltersPayload = (
    filters: OverseasPaymentFilters,
    searchValue: string,
  ) => {
  ...
  const {
    ...
    error: paymentError,
  } = useQuery<OverseasPaymentListQueryResult>({
    queryKey: [
      "overseas-payment",
    ...
    queryFn: async (): Promise<OverseasPaymentListQueryResult> => {
      try {
        ...
        const body = bodyCandidate != null ? (bodyCandidate as PaymentFilterResponse | PaymentRow[]) : null;
        if (!body) {
          setTotalRecords(0);
          return { data: [], summary: undefined };
        }
        const list = Array.isArray((body as PaymentFilterResponse).data)
          ? ((body as PaymentFilterResponse).data as PaymentRow[])
          : Array.isArray(body) ? (body as PaymentRow[]) : [];
        const totalEnvelope =
          body != null &&
          typeof body === "object" &&
          !Array.isArray(body) &&
        ...
        const summary: OverseasPaymentListSummary | undefined =
          rawSummary && typeof rawSummary === "object" && !Array.isArray(rawSummary)
            ? (rawSummary as OverseasPaymentListSummary)
            : undefined;
  ...
  const paymentData = paymentListResult?.data ?? [];
  ...
  const isLoading = paymentFetching || paymentLoading || isInitialLoad;
  ...
  const listStats = useMemo(() => {
    ...
    const summary = paymentListResult?.summary;
    if (summary) {
  ...
  }, [tableData, paymentListResult?.summary, totalRecords]);

  const filterFieldStyles = erpListFilterUnifiedMantineStyles(erpTheme);

  const columnToggleItems = useMemo(
    () =>
      (Object.keys(visibleColumns) as (keyof OverseasPaymentColumnVisibility)[]).map(
        (key) => ({
          id: String(key),
          label: overseasPaymentColumnLabels[key],
  ...
  const allColumns = useMemo<MRT_ColumnDef<PaymentRow>[]>(
    () => [
      {
        id: "sno",
        header: "S.No",
  ...
  const columns = useMemo(
    () =>
      allColumns.filter((col) => {
        const id = overseasPaymentColumnId(col);
        if (id === "actions") return true;
        return visibleColumns[id as keyof OverseasPaymentColumnVisibility] !== false;
      }),
        ...
        {isLoading ? (
          <Stack align="center" gap="md">
            ...
            <Text c="dimmed" size="sm" style={{ fontFamily: erpTheme.fontSans }}>
              Loading overseas payment data…
            </Text>
          </Stack>
        ) : (
          <Text c="dimmed" size="sm" style={{ fontFamily: erpTheme.fontSans }}>
            No overseas payment data found
          </Text>
        )}
      </Center>
    ),
      ...
      >
        <ERPListScreen
              ...
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setStoreSearch(LIST_KEY, search);
                    setShouldRestore(LIST_KEY, true);
                    navigate("/overseas-payment/create");
                  }}
                >
                  Create New
                </Button>
          ...
          filters={{
            opened: showFilters,
            title: "Filters",
            subtitle: "Overseas (agent) payments — refine by day book, payment no., dates, type, or status",
                ...
                <Grid.Col span={ERP_LIST_FILTER_FIELD_COL_SPAN}>
                  <Box style={erpListFilterFieldCellStyle}>
                    <SearchableSelect
                      ...
                      onChange={(val) => setDraftFilters((prev) => ({ ...prev, payment_no: val || "" }))}
                      dropdownZIndex={1000}
                      minSearchLength={1}
            ...
            children: paymentError ? (
              <Center py="xl" style={{ backgroundColor: cardBg, flex: 1, minHeight: 200 }}>
                <Text size="sm" c="dimmed" style={{ fontFamily: erpTheme.fontSans }}>
                  Error loading overseas payment data. Please try refreshing the page.
                </Text>
              </Center>
            ) : (
              /*
