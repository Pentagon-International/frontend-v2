...
const validationSchema = yup.object({
  ...
  delivery_address_id: yup.number(),
...
const ImportShipmentStepper: React.FC<ImportShipmentStepperProps> = ({
  onStepChange,
  onComplete,
  initialData,
  isEditMode = false,
  ...
  const mapInitialDataToFormValues = (
    ...
    return {
      // Import Shipment fields
      // For edit mode, if codes are not available, we'll use names as codes temporarily
      // The dropdowns will handle the display properly
      customer_code: String(data.customer_code_read || ""),
      customer_name: String(data.customer_name || ""),
      service: String(data.service || ""),
      date: data.date ? new Date(String(data.date)) : new Date(),
      origin_code: String(data.origin_code_read || ""),
      origin_name: String(data.origin_name || ""),
      destination_code: String(data.destination_code_read || ""),
      ...
      destination_agent_name: String(data.destination_agent_name || ""),
      ...
      billing_customer_name: String(data.billing_customer_name || ""),
      ...
      notify_customer_name: String(data.notify_customer_name || ""),
      ...
      pickup_from_name: String(data.pickup_from_name || ""),
      pickup_address_id: Number(data.pickup_address_id) || 0,
      pickup_address_name: String(data.pickup_address_name || ""),
      ...
      delivery_from_name: String(data.delivery_from_name || ""),
      delivery_address_id: Number(data.delivery_address_id) || 0,
      delivery_address_name: String(data.delivery_address_name || ""),
  ...
  useEffect(() => {
    if (isEditMode && initialData) {
      console.log("=== ImportShipmentStepper Debug ===");
      ...
      // Handle quotation charges if available
      if (
        initialData.quotation_charges &&
        Array.isArray(initialData.quotation_charges)
      ) {
        const mappedCharges = initialData.quotation_charges.map(
  ...
  // Initialize routing display names
  React.useEffect(() => {
    if (routingDisplayNames.length === 0) {
      setRoutingDisplayNames([{ from: null, to: null, carrier: null }]);
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        // Routing Details
        routing_details: form.values.routingDetails.map((detail) => ({
          move_type: detail.move_type,
          from_location_code: detail.from_location_code,
          to_location_code: detail.to_location_code,
      ...
      (payload as Record<string, unknown>).service_type = "IMPORT";
  ...
  return (
    <Box px={"lg"}>
      ...
      >
        {/* Step 1: Import Shipment */}
        <Stepper.Step label="1" description="Import Booking">
          <Box mt="md">
                ...
                <SearchableSelect
                  ...
                  displayValue={
                    ...
                    form.values.customer_code
                      ? `${form.values.customer_name} (${form.values.customer_code})`
                      : form.values.customer_name
                  }
              ...
              {form.values.routingDetails.map((_, index) => (
                <Box key={index}>
                  <Grid>
                    <Grid.Col span={1.25}>
                      <Dropdown
                        data={["Value", "Prepaid", "Collect", "Third Party"]}
                      ...
                      <SearchableSelect
                        ...
                        onChange={(value, selectedData) => {
                          ...
                          if (value && selectedData) {
                            ...
                            setRoutingDisplayNames((prev) => {
                              const updated = [...prev];
                              updated[index] = { ...updated[index], to: null };
                              return updated;
                            });
                          }
                        }}
            ...
            <Grid mb="md">
              <Grid.Col span={4}>
                <SearchableSelect
                  ...
                  displayValue={form.values.destination_agent_name}
                  onChange={(value, selectedData, originalData) => {
            ...
            <Grid mb="md">
              <Grid.Col span={4}>
                <SearchableSelect
                  ...
                  displayValue={form.values.notify_customer_name}
                  onChange={(value, selectedData, originalData) => {
              ...
              <Grid.Col span={4}>
                <SearchableSelect
                  ...
                  onChange={(value, selectedData, originalData) => {
                    ...
                    form.setFieldValue("cha_name", selectedData?.label || "");
            ...
            <Grid style={{ maxWidth: "80%" }}>
              ...
              <Grid.Col span={6}>
                <SearchableSelect
                  ...
                  displayValue={form.values.pickup_from_name}
                  onChange={(value, selectedData) => {
              ...
              <Grid.Col span={6}>
                <SearchableSelect
                  ...
                  displayValue={form.values.pickup_address_name}
                  onChange={(value, selectedData) => {
                    form.setFieldValue(
                      "pickup_address_id",
                      value ? parseInt(value) : 0
              ...
              <Grid.Col span={6}>
                <SearchableSelect
                  ...
                  displayValue={form.values.delivery_from_name}
                  onChange={(value, selectedData) => {
              ...
              <Grid.Col span={6}>
                <SearchableSelect
                  ...
                  displayValue={form.values.delivery_address_name}
                  onChange={(value, selectedData) => {
                    form.setFieldValue(
                      "delivery_address_id",
                      value ? parseInt(value) : 0
...
export default ImportShipmentStepper;
