...
function ExportShipmentMaster() {
  ...
  // Fetch export shipments data using filter endpoint with empty body
  const {
    data: exportShipmentsData = [],
    isLoading,
    refetch: refetchExportShipments,
  } = useQuery({
    queryKey: ["export-shipment/filter/"],
    queryFn: async () => {
      try {
  ...
  const {
    ...
    queryKey: ["filteredExportShipments"],
    queryFn: async () => {
  ...
  const { data: searchData, isLoading: searchLoading } = useQuery({
    queryKey: ["exportSearch", debounced],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (shouldRefetch) {
      ...
      queryClient.invalidateQueries({ queryKey: ["export-shipment/filter/"] });
  ...
  useEffect(() => {
    ...
    if (location.state?.refreshData) {
      ...
      const refreshData = async () => {
        try {
          console.log(
            "🔄 Starting aggressive data refresh for export shipments..."
          );

          // Remove all cached data first
          queryClient.removeQueries({ queryKey: ["export-shipment/filter/"] });
          queryClient.removeQueries({ queryKey: ["filteredExportShipments"] });
          ...
          if (
            filtersApplied &&
            Object.keys(buildFilterPayload(appliedFilters)).length > 0
          ) {
          ...
          setTimeout(async () => {
            ...
            console.log(
              "✅ Export shipment data refresh completed with additional refetch"
            );
          }, 200);

          console.log("✅ Export shipment data refresh completed");
        } catch (error) {
          console.error("Error refreshing export shipment data:", error);
        }
      };
  ...
  useEffect(() => {
    const refreshOnMount = async () => {
      try {
        ...
        console.log("🔄 Export shipment data refreshed on component mount");
      } catch (error) {
        console.error("Error refreshing export shipment data on mount:", error);
      }
    };
  ...
  const clearAllFilters = async () => {
    try {
      ...
      queryClient.removeQueries({ queryKey: ["filteredExportShipments"] }); // Remove filtered data from cache
      ToastNotification({
        type: "success",
  ...
  const columns = useMemo<MRT_ColumnDef<ExportShipmentData>[]>(
    () => [
      ...
      //         <Box px={10} py={5}>
      //           <UnstyledButton
      //             onClick={() => handleDeleteExportShipment(row.original)}
      //           >
      //             <Group gap={"sm"}>
          ...
          <Center py="xl">
            <Stack align="center" gap="md">
              ...
              {/* <Text c="dimmed" size="sm">
                Create your first export shipment to get started
              </Text> */}
            </Stack>
          </Center>
        </td>
                    ...
                    <Grid.Col span={2.4}>
                      <SearchableSelect
                        ...
                        displayFormat={(item: Record<string, unknown>) => ({
                          ...
                          // label: `${item.customer_name} (${item.port_code})`,
...
export default ExportShipmentMaster;
