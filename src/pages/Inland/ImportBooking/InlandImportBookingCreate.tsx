...
import InlandImportBookingStepper from "./InlandImportBookingStepper";
...
function InlandImportBookingCreate() {
  ...
  const mapBookingDataToStepperFormat = (data: Record<string, unknown>) => {
    ...
    return {
      ...
      service: "INLAND",
      service_code: String(serviceDetails.service_code ?? quotationData.service_code ?? ""),
      service_name: String(serviceDetails.service_name ?? quotationData.service_name ?? ""),
      ...
      notify1_customer_address: 0,
      notify1_customer_email: "",
      notify2_customer_name: "",
      notify2_customer_address: 0,
  ...
  // Debug logging
  console.log("InlandImportBookingCreate - mode:", mode);
  console.log("InlandImportBookingCreate - jobData:", jobData);
  console.log("InlandImportBookingCreate - bookingData:", bookingData);
  console.log("InlandImportBookingCreate - mappedBookingData:", mappedBookingData);
  ...
  const steps = [
    { label: "Import Booking", icon: IconUser },
  ...
  const handleComplete = () => {
    console.log("Inland Import Booking creation completed!");
    ...
    async (bookingMessage: string, bookingId: number) => {
      ...
      try {
        ...
        if (response?.success && bookingItem) {
          navigate("/inland/import-booking/edit", {
  ...
  // When navigated from Customer Service dashboard with bookingId only - fetch and show booking
  useEffect(() => {
          ...
          >
            ...
            >
              <EditPageHeadingRow
                ...
                ariaLabel="Inland import booking audit info"
              ...
              >
                ...
                >
                  {isEditMode
                    ? "Edit Inland Import Booking"
                    : "Create Inland Import Booking"}
                </Text>
              </EditPageHeadingRow>
            </Box>
...
export default InlandImportBookingCreate;
