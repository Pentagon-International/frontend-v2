...
const INLAND_IMPORT_GEIST_ROOT_CLASS = "inland-import-booking-geist-root";
const INLAND_IMPORT_GEIST_MONO_CLASS = "inland-import-geist-mono";
...
const v0ModalClassNames = {
  content: INLAND_IMPORT_GEIST_ROOT_CLASS,
  body: INLAND_IMPORT_GEIST_ROOT_CLASS,
  inner: INLAND_IMPORT_GEIST_ROOT_CLASS,
};
...
const AIR_EXPORT_FILTER_SELECT_CLASSNAMES = {
  dropdown: INLAND_IMPORT_GEIST_ROOT_CLASS,
  option: INLAND_IMPORT_GEIST_ROOT_CLASS,
};
...
function getMilestoneDrawerDetail(
  ...
  switch (index) {
    ...
    case 2:
      return { detail: "Received at import facility / terminal", when: "—" };
    case 3:
      return {
        detail: String(oc),
        when: row.atd
...
// ---------- Main Component ----------
function InlandImportBookingMaster() {
  ...
  const handleExport = (rows: ExportShipmentData[]) => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Import Bookings");
    XLSX.writeFile(
      wb,
      `inland_import_bookings_${dayjs().format("YYYY-MM-DD_HH-mm")}.xlsx`,
    );
  };
  ...
  const RowMenu = ({ row }: { row: ExportShipmentData }) => {
    ...
    return (
      <Menu
        ...
        position="bottom-end"
        styles={v0MenuStyles}
        classNames={{ dropdown: INLAND_IMPORT_GEIST_ROOT_CLASS }}
      >
        <Menu.Target>
  ...
  return (
    <MantineProvider theme={airExportV0MantineTheme}>
      <Box className={INLAND_IMPORT_GEIST_ROOT_CLASS} style={v0RootTypography}>
        {showMasterTable && (
          <ERPListScreen
            theme={erpTheme}
            toolbar={{
                      ...
                      tableRows.map((booking) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {visibleColumns.job_no && (
                              <td
                                className={INLAND_IMPORT_GEIST_MONO_CLASS}
                                style={{
        ...
        <Drawer
          ...
          classNames={{
            content: INLAND_IMPORT_GEIST_ROOT_CLASS,
            body: INLAND_IMPORT_GEIST_ROOT_CLASS,
            header: INLAND_IMPORT_GEIST_ROOT_CLASS,
          }}
          styles={{
            content: { fontFamily: V0_FONT_SANS },
...
export default InlandImportBookingMaster;
