...
const LIST_KEY = "CUSTOMER_TYPE_MASTER";
...
const DEFAULT_FILTERS: CustomerTypeFilters = {
  customer_type_code: "",
  customer_type_name: "",
  customer_category: "",
  status: "",
...
const formatCategoryLabel = (value?: string) => {
  if (!value) return "-";
  const match = CUSTOMER_CATEGORY_OPTIONS.find(
...
export default function CustomerTypeMaster() {
  ...
  const [draftFilters, setDraftFilters] =
    useState<CustomerTypeFilters>(DEFAULT_FILTERS);
  ...
  const persistFiltersBeforeNavigate = useCallback(() => {
  ...
  const buildFiltersPayload = (
    ...
    if (trimmedSearch) cleaned.search = trimmedSearch;

    if (Object.keys(cleaned).length === 0 && !trimmedSearch) {
      return {};
    }
  ...
  const {
    ...
    isFetching: customerTypeFetching,
    ...
    queryFn: async () => {
      try {
        ...
        const response = await apiCallProtected.post(
          `${URL.customerTypeFilter}?index=${index}&limit=${pagination.pageSize}`,
          payload,
          API_HEADER,
        );
  ...
  }, [location.state?.refreshData, refetch]);

  const isLoading =
    customerTypeFetching || customerTypeLoading || isInitialLoad;
  const tableData = customerTypeData ?? [];
  ...
  const columns = useMemo<MRT_ColumnDef<CustomerType>[]>(
    () => [
      ...
      },
      {
        ...
        minSize: 140,
        Cell: ({ cell }) => formatCategoryLabel(cell.getValue<string>()),
      },
      {
        accessorKey: "status",
        header: "Status",
    ...
    [navigate, persistFiltersBeforeNavigate],
  );
          ...
          <Group gap="xs" wrap="nowrap">
            ...
            {isAdmin && (
              <Button
                ...
                onClick={() => {
                  persistFiltersBeforeNavigate();
                  navigate("/master/customer-type-new");
                }}
              >
                Create New
              </Button>
          ...
          <Grid gutter="sm" px="md" pt="xs" pb="sm">
            <Grid.Col span={2.4}>
              <FormTextInput
                ...
                value={draftFilters.customer_type_code}
                onChange={(e) =>
                  setDraftFilters((prev) => ({
                    ...prev,
                    customer_type_code: e.currentTarget.value,
                  }))
                }
              />
            </Grid.Col>
            ...
            <Grid.Col span={2.4}>
              <FormTextInput
                ...
                value={draftFilters.customer_type_name}
                onChange={(e) =>
                  setDraftFilters((prev) => ({
                    ...prev,
                    customer_type_name: e.currentTarget.value,
                  }))
                }
              />
            </Grid.Col>
            ...
            <Grid.Col span={2.4}>
              <Dropdown
                ...
                searchable
                value={draftFilters.customer_category || null}
                onChange={(value) =>
                  setDraftFilters((prev) => ({
                    ...prev,
      ...
      {isLoading ? (
        <Center py="xl" style={{ flex: 1 }}>
          <Stack align="center" gap="md">
            ...
            <Text c="dimmed">Loading Customer Type data...</Text>
          </Stack>
        </Center>
      ) : customerTypeError ? (
        <Center py="xl" style={{ flex: 1 }}>
          <Stack align="center" gap="md">
            <Text c="dimmed">
              Error loading Customer Type data. Please try refreshing the page.
            </Text>
          </Stack>
        </Center>
      ) : (
