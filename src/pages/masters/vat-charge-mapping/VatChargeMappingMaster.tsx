...
const LIST_KEY = "VAT_CHARGE_MAPPING_MASTER";

type VatChargeMappingRow = {
  id?: number;
  sno?: number;
  service_id?: number;
  service_name?: string;
...
export default function VatChargeMappingMasterList() {
  ...
  const DEFAULT_FILTERS: VatChargeMappingFilters = {
    vat_id: "",
    vat_code: "",
    service_id: "",
    service_name: "",
  ...
  const [draftFilters, setDraftFilters] =
    useState<VatChargeMappingFilters>(DEFAULT_FILTERS);
  ...
  const buildFiltersPayload = (
    filters: VatChargeMappingFilters,
    searchValue: string,
  ) => {
    const cleaned: Record<string, string | number> = {};
    if (filters.vat_id?.trim()) cleaned.vat_id = Number(filters.vat_id);
    if (filters.service_id?.trim())
      cleaned.service_id = Number(filters.service_id);
    if (filters.charge_id?.trim()) cleaned.charge_id = Number(filters.charge_id);
  ...
  const {
    data: vatChargeMappingData = [],
    isLoading: vatChargeMappingLoading,
    isFetching: vatChargeMappingFetching,
    error: vatChargeMappingError,
  } = useQuery({
    queryKey: [
      "vat-charge-mapping",
    ...
    queryFn: async () => {
      try {
        ...
        const response = await apiCallProtected.post(
          `${URL.vatChargeMappingFilter}?index=${index}&limit=${pagination.pageSize}`,
          payload,
        );
        setIsInitialLoad(false);
        setShowFilters(false);

        const data = response as { data?: VatChargeMappingRow[]; total?: number };
        ...
        console.error("Error fetching VAT Charge Mapping data:", error);
        setShowFilters(false);
        setTotalRecords(0);
        throw error;
      }
  ...
  const isLoading =
    vatChargeMappingFetching || vatChargeMappingLoading || isInitialLoad;
  const tableData = vatChargeMappingData ?? [];

  const columns = useMemo<MRT_ColumnDef<VatChargeMappingRow>[]>(
    () => [
      {
        accessorKey: "sno",
        header: "S.No",
      ...
      { accessorKey: "vat_code", header: "VAT Code", size: 120 },
      { accessorKey: "vat_name", header: "VAT Name", size: 180 },
      {
              ...
              <Box px={10} py={5}>
                <UnstyledButton
                  onClick={() => {
                    ...
                    navigate("/master/vat-charge-mapping/edit", {
                      state: row.original,
                    });
                  }}
                >
    ...
    >
      ...
      {showFilters && (
        ...
        >
          ...
          >
            <Text size="sm" fw={600} c="#1E293B" style={{ fontFamily: "Inter" }}>
              Filter
            </Text>
            <ActionIcon
              variant="subtle"
          ...
          <Grid gutter="sm" px="md" pt="xs" pb="sm">
            <Grid.Col span={4}>
              <SearchableSelect
                ...
                value={draftFilters.vat_id}
                displayValue={draftFilters.vat_code || ""}
                onChange={(val, option) =>
                  setDraftFilters((prev) => ({
                    ...prev,
                    vat_id: val || "",
                    vat_code: option?.label || "",
                  }))
                }
                dropdownZIndex={1000}
                minSearchLength={1}
      ...
      {isLoading ? (
        <Center py="xl" style={{ flex: 1 }}>
          <Stack align="center" gap="md">
            ...
            <Text c="dimmed">Loading VAT Charge Mapping data...</Text>
          </Stack>
        </Center>
      ) : vatChargeMappingError ? (
        <Center py="xl" style={{ flex: 1 }}>
          <Text c="dimmed">
            Error loading VAT Charge Mapping data. Please try refreshing the page.
          </Text>
        </Center>
      ) : (
        <>
