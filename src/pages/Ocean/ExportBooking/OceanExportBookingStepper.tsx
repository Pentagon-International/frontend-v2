...
const OceanExportBookingStepper: React.FC<ExportShipmentStepperProps> = ({
  onStepChange,
  onComplete,
  initialData,
  isEditMode = false,
    ...
    setCharges((prev) =>
      prev.map((charge, i) => {
        if (i === index) {
          ...
          // Calculate total_cost when relevant fields change
  ...
  const form = useForm<FormValues>({
    ...
    initialValues: {
      ...
      trigger_modal_rows: [{ type: null, code: null, description: "" }],
  ...
  const handleShipperCustomerChange = ({
    ...
    // Keep shipper_code empty for freeText so address stays manually editable.
    // For temp, store temp_code on shipper_code for job/booking identity.
    form.setFieldValue(
      "shipper_code",
    ...
    if (selectionType === "temp" && originalData) {
      ...
      // Preserve address/email if user is still typing the name; clear only when empty name
      if (!name.trim()) {
        form.setFieldValue("shipper_address", "");
        form.setFieldValue("shipper_email", "");
      }
  ...
  // immediately when origin/destination are already present.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // In edit mode, keep the saved schedule/routings values visible.
      if (!preserveSaved) {
        form.setFieldValue("schedule_id", "");
      }
      ...
      try {
        ...
        // Ensure the saved schedule_id shows up in dropdown options for the initial edit fetch.
    ...
    queryFn: () =>
      isFromQuotationFlow
        ...
        : fetchQuotations({
            ...
            service_type: "Export", // Ocean Export
          }),
  ...
  useEffect(() => {
    ...
    if (jobData.shipper_name) {
      ...
      const hasShipperCode = Boolean(
        String(
          jobData.shipper_code ||
            (mappedData as { shipper_code?: string }).shipper_code ||
            "",
        ).trim(),
      );
      const hasFwd =
        Boolean(String(jobData.forwarder_code || "").trim()) ||
        Boolean(String(jobData.forwarder_name || "").trim()) ||
        Boolean(
          String(
            (mappedData as { forwarder_code?: string }).forwarder_code || "",
          ).trim(),
        ) ||
        Boolean(
          String(
            (mappedData as { forwarder_name?: string }).forwarder_name || "",
          ).trim(),
        );
    ...
    if (jobData.notify1_customer_address && !jobData.notify1_customer_name) {
  ...
  useEffect(() => {
    ...
    if (initialData.shipper_name) {
      ...
      const hasShipperCode = Boolean(
        String(initialData.shipper_code || "").trim(),
      );
      const hasFwd =
    ...
    // Notify Customer 1 address options when we have address but didn't set from name
    if (
    ...
    <>
      ...
      >
        <Box style={{ padding: "24px 24px 32px" }}>
          ...
          {/* Added events / documents / trigger updates (display until submit) */}
          {/* {(form.values.events.length > 0 ||
            form.values.document_display_list.length > 0 ||
            form.values.trigger_updates.length > 0) && (
            <Box
              mb="md"
              p="sm"
              style={{
              ...
              <Stack gap="xs">
                {form.values.events.length > 0 && (
                  <Box>
                    ...
                    {form.values.events.map((e, i) => (
                      <Text key={i} size="xs" ml="sm">
                        {e.type} – {e.date || "—"}
                      </Text>
                    ))}
                  </Box>
                ...
                {form.values.document_display_list.length > 0 && (
                  <Box>
                    ...
                    {form.values.document_display_list.map((d) => (
                      <Text key={d.id} size="xs" ml="sm">
                        {d.documentName}
                        {d.userFileName ? ` (${d.userFileName})` : ""}
                      </Text>
                    ))}
                  </Box>
                )}
                {form.values.trigger_updates.length > 0 && (
                  <Box>
                    ...
                    {form.values.trigger_updates.map((t, i) => (
                      <Text key={i} size="xs" ml="sm">
                        {t.type} – {t.code}
...
export default OceanExportBookingStepper;
