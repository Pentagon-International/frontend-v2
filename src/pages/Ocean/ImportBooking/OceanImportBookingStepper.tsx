...
const OceanImportBookingStepper: React.FC<ImportShipmentStepperProps> = ({
  ...
  >(initialRoutingDisplayNames);

  // Sync routingDisplayNames when initialRoutingDisplayNames changes (e.g., when initialData loads)
  React.useEffect(() => {
    if (
      initialRoutingDisplayNames.length > 0 &&
      routingDisplayNames.length === 0
    ) {
      setRoutingDisplayNames(initialRoutingDisplayNames);
    }
  }, [initialRoutingDisplayNames, routingDisplayNames.length]);
  ...
  const mapInitialDataToFormValues = (
    ...
    return {
      ...
      // Store codes in form values for payload submission
  ...
  useEffect(() => {
    if (isEditMode && initialData) {
      ...
      // Set routing display names - only if not already initialized from useMemo
      // This ensures display names are set even if initialData changes
      if (
        initialData.routing_details &&
        Array.isArray(initialData.routing_details) &&
        initialData.routing_details.length > 0 &&
  ...
  // Initialize routing display names to match routingDetails length (only for non-edit mode or when adding new routes)
  React.useEffect(() => {
    // Skip this effect if we're in edit mode and have initialData - the main useEffect handles that
    if (isEditMode && initialData?.routing_details) {
      return;
    }

    if (form.values.routingDetails && form.values.routingDetails.length > 0) {
      // Only initialize if the lengths don't match
      if (routingDisplayNames.length !== form.values.routingDetails.length) {
        // If we already have some display names, preserve them and pad/trim as needed
        const currentNames = [...routingDisplayNames];
        const newLength = form.values.routingDetails.length;

        if (currentNames.length < newLength) {
          // Add null entries for new routing details
          const padding = Array(newLength - currentNames.length)
            .fill(null)
            .map(() => ({
              from: null,
              to: null,
          ...
          setRoutingDisplayNames([...currentNames, ...padding]);
        } else if (currentNames.length > newLength) {
          // Trim excess entries
          setRoutingDisplayNames(currentNames.slice(0, newLength));
        }
      }
    } else if (routingDisplayNames.length > 0) {
      // Clear display names if routingDetails is empty
      setRoutingDisplayNames([]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.values.routingDetails, isEditMode, initialData]);
            ...
            <Grid.Col span={4}>
              <SearchableSelect
                ...
                onChange={(value, selectedData) => {
                  ...
                  form.setFieldValue(
                    "origin_name",
                    selectedData?.label ? selectedData.label.split(" (")[0] : ""
                  );
                }}
            ...
            <Grid.Col span={4}>
              <SearchableSelect
                ...
                onChange={(value, selectedData, originalData) => {
                  ...
                  setShipperDisplayName(selectedData?.label || null);
            ...
            <Grid.Col span={4}>
              <SearchableSelect
                ...
                onChange={(value, selectedData, originalData) => {
                  ...
                  setConsigneeDisplayName(selectedData?.label || null);
            ...
            <Grid.Col span={4}>
              <SearchableSelect
                ...
                onChange={(value, selectedData, originalData) => {
                  ...
                  setDestinationAgentDisplayName(selectedData?.label || null);
            ...
            <Grid.Col span={4}>
              <SearchableSelect
                ...
                onChange={(value, selectedData, originalData) => {
                  ...
                  setBillingCustomerDisplayName(selectedData?.label || null);
            ...
            <Grid.Col span={6}>
              <SearchableSelect
                ...
                onChange={(value, selectedData) => {
                  ...
                  setDeliveryAddressDisplayName(selectedData?.label || null);
                }}
            ...
            <Grid.Col span={4}>
              <Dropdown
                ...
                onChange={(value) => {
                  ...
                  if (value && quotationsData?.status && quotationsData.data) {
