...
function OceanImportBookingMaster() {
  ...
  const handleCreateJob = async (booking: ImportShipmentData & Record<string, unknown>) => {
    const routingDetails = Array.isArray(booking.routing_details) ? (booking.routing_details as Array<Record<string, unknown>>) : [];
    const cargoDetails = Array.isArray(booking.cargo_details) ? (booking.cargo_details as Array<Record<string, unknown>>) : [];
    const rateDetails = Array.isArray(booking.rate_details) ? (booking.rate_details as Array<Record<string, unknown>>) : [];
    const containerDetails = Array.isArray(booking.container_details) ? (booking.container_details as Array<Record<string, unknown>>) : [];
    const housingDetails = Array.isArray(booking.housing_details) ? (booking.housing_details as Array<Record<string, unknown>>) : [];

    const payload: Record<string, unknown> = {
      service: booking.service || "FCL",
      service_type: booking.service_type || "Import",
      agent: (booking.destination_agent_code as string) || (booking.destination_agent_name as string) || "",
      ...
      atd: (booking.atd as string) ? dayjs(booking.atd as string).isValid() ? dayjs(booking.atd as string).format("YYYY-MM-DD") : null : null,
      ata: (booking.ata as string) ? dayjs(booking.ata as string).isValid() ? dayjs(booking.ata as string).format("YYYY-MM-DD") : null : null,
      igm_no: (booking.igm_no as string) || null,
      igm_date: (booking.igm_date as string) ? dayjs(booking.igm_date as string).isValid() ? dayjs(booking.igm_date as string).format("YYYY-MM-DD") : null : null,
      carrier_code: (booking.carrier_code as string) || "",
      vessel_name: booking.vessel_name || "",
      voyage_number: booking.voyage_no || "",
      mbl_number: (booking.mbl_no as string) || "",
      mbl_date: (booking.mbl_date as string) ? dayjs(booking.mbl_date as string).isValid() ? dayjs(booking.mbl_date as string).format("YYYY-MM-DD") : null : null,
      shipper_name: booking.shipper_name || "",
      shipper_email: (booking.shipper_email as string) || "",
      shipper_address: (booking.shipper_address as string) || "",
      consignee_name: booking.consignee_name || "",
      consignee_email: (booking.consignee_email as string) || "",
      consignee_address: (booking.consignee_address as string) || "",
      carrier_agent_name: (booking.carrier_agent_name as string) || "",
      carrier_agent_email: (booking.carrier_agent_email as string) || "",
      carrier_agent_address: (booking.carrier_agent_address as string) || "",
      booking_ids: [],
      ocean_routings: routingDetails.map((r) => ({
        transport_type: (r.move_type as string) || "SEA",
        from_port_code: (r.from_port_code as string) || "",
        to_port_code: (r.to_port_code as string) || "",
        etd: (r.etd as string) ? dayjs(r.etd as string).isValid() ? dayjs(r.etd as string).format("YYYY-MM-DD") : null : null,
        eta: (r.eta as string) ? dayjs(r.eta as string).isValid() ? dayjs(r.eta as string).format("YYYY-MM-DD") : null : null,
        atd: (r.atd as string) ? dayjs(r.atd as string).isValid() ? dayjs(r.atd as string).format("YYYY-MM-DD") : null : null,
        ata: (r.ata as string) ? dayjs(r.ata as string).isValid() ? dayjs(r.ata as string).format("YYYY-MM-DD") : null : null,
        carrier_code: (r.carrier_code as string) || null,
        vessel: (r.vessel_name as string) || null,
        flight: null,
        truck_no: null,
        rail_no: null,
        voyage_number: (r.voyage_no as string) || null,
      })),
      housing_details: housingDetails.length > 0 ? housingDetails.map((h) => ({
        hbl_number: (h.hbl_number as string) || "",
        house_date: (h.house_date as string) ? dayjs(h.house_date as string).isValid() ? dayjs(h.house_date as string).format("YYYY-MM-DD") : null : null,
        routed: (h.routed as string) || booking.routed || "",
        routed_by: (h.routed_by as string) || booking.routed_by || "",
        origin_code: (h.origin_code as string) || booking.origin_code_read || "",
        destination_code: (h.destination_code as string) || booking.destination_code_read || "",
        customer_service: (h.customer_service as string) || booking.customer_service_name || "",
        trade: "Import",
        agent_name: (h.agent_name as string) || (booking.destination_agent_name as string) || "",
        agent_address: (h.agent_address as string) || (booking.destination_agent_address as string) || "",
        agent_email: (h.agent_email as string) || (booking.destination_agent_email as string) || "",
        cha_name: (h.cha_name as string) || booking.cha_name || "",
        cha_address: (h.cha_address as string) || (booking.cha_address as string) || "",
        shipper_name: (h.shipper_name as string) || booking.shipper_name || "",
        shipper_address: (h.shipper_address as string) || (booking.shipper_address as string) || "",
        shipper_email: (h.shipper_email as string) || (booking.shipper_email as string) || "",
        shipper_state_id: (h.shipper_state_id as string) || "",
        consignee_name: (h.consignee_name as string) || booking.consignee_name || "",
        consignee_address: (h.consignee_address as string) || (booking.consignee_address as string) || "",
        consignee_email: (h.consignee_email as string) || (booking.consignee_email as string) || "",
        notify1_customer_name: (h.notify1_customer_name as string) || (booking.notify_customer_name as string) || "",
        notify1_customer_address: (h.notify1_customer_address as string) || (booking.notify_customer_address as string) || "",
        notify1_customer_email: (h.notify1_customer_email as string) || (booking.notify_customer_email as string) || "",
        commodity_description: (h.commodity_description as string) || booking.commodity_description || "",
        marks_no: (h.marks_no as string) || booking.marks_no || "",
        item_no: "",
        sub_item_no: "",
        shipment_terms_code: (h.shipment_terms_code as string) || booking.shipment_terms_code_read || "",
        events: [],
        cargo_details: Array.isArray(h.cargo_details) ? (h.cargo_details as Array<Record<string, unknown>>).map((cargo) => ({
          ...
          haz: Boolean(cargo.haz) || booking.is_hazardous || false,
        })) : cargoDetails.map((cargo) => ({
          container_no: String((cargo.container_no as string) || ""),
          no_of_packages: Number((cargo.no_of_packages as number)) || 0,
          gross_weight: Number(cargo.gross_weight) || 0,
          volume: Number((cargo.volume as number)) || 0,
          chargeable_weight: Number((cargo.chargeable_weight as number)) || 0,
          haz: booking.is_hazardous || false,
        })),
        mbl_charges: Array.isArray(h.mbl_charges) ? (h.mbl_charges as Array<Record<string, unknown>>).map((charge) => ({
          charge_id: Number(charge.charge_id) || "",
          supplier_code: String(charge.supplier_code || ""),
          pp_cc: String(charge.pp_cc || "Prepaid"),
          unit_id: Number(charge.unit_id) || 0,
          currency_id: Number(charge.currency_id) || 0,
          no_of_unit: Number(charge.no_of_unit) || 0,
          roe: Number(charge.roe) || 1,
          amount_per_unit: Number(charge.amount_per_unit) || 0,
          amount: Number(charge.amount) || 0,
          sell_local_amount: Number(charge.sell_local_amount) || 0,
          unit_cost: Number(charge.unit_cost) || 0,
          total_cost: Number(charge.total_cost) || 0,
          cost_local_amount: Number(charge.cost_local_amount) || 0,
        })) : rateDetails.map((charge) => ({
          charge_id: Number((charge.charge_id as number)) || "",
          supplier_code: "",
          pp_cc: String((charge.pp_cc as string) || ""),
          unit_id: Number((charge.unit_id as number)) || 0,
          currency_id: Number((charge.currency_id as number)) || 0,
          no_of_unit: Number(charge.no_of_unit) || 0,
          roe: Number((charge.roe as number)) || "",
          amount_per_unit: Number((charge.sell_per_unit as number)) || "",
          amount: Number(charge.sell_amount_total) || "",
          sell_local_amount: Number(charge.sell_amount_total) || "",
          unit_cost: Number((charge.cost_per_unit as number)) || "",
          total_cost: Number((charge.total_cost as number)) || "",
          cost_local_amount: Number((charge.total_cost as number)) || "",
        })),
      })) : [{
        hbl_number: "",
        ...
        trade: "Import",
        agent_name: (booking.destination_agent_name as string) || "",
        agent_address: (booking.destination_agent_address as string) || "",
        agent_email: (booking.destination_agent_email as string) || "",
        cha_name: booking.cha_name || "",
        cha_address: (booking.cha_address as string) || "",
        shipper_name: booking.shipper_name || "",
        shipper_address: (booking.shipper_address as string) || "",
        shipper_email: (booking.shipper_email as string) || "",
        shipper_state_id: "",
        ...
        consignee_address: (booking.consignee_address as string) || "",
        consignee_email: (booking.consignee_email as string) || "",
        notify1_customer_name: (booking.notify_customer_name as string) || "",
        notify1_customer_address: (booking.notify_customer_address as string) || "",
        notify1_customer_email: (booking.notify_customer_email as string) || "",
        ...
        mbl_charges: rateDetails.map((charge) => ({
          charge_id: Number((charge.charge_id as number)) || "",
          supplier_code: "",
          pp_cc: String((charge.pp_cc as string) || "Prepaid"),
          unit_id: Number((charge.unit_id as number)) || 0,
          currency_id: Number((charge.currency_id as number)) || 0,
          no_of_unit: Number(charge.no_of_unit) || 0,
          roe: Number((charge.roe as number)) || 1,
          amount_per_unit: Number((charge.sell_per_unit as number)) || 0,
          amount: Number(charge.sell_amount_total) || 0,
          sell_local_amount: Number(charge.sell_amount_total) || 0,
          unit_cost: Number((charge.cost_per_unit as number)) || 0,
          total_cost: Number((charge.total_cost as number)) || 0,
          cost_local_amount: Number((charge.total_cost as number)) || 0,
        })),
      }],
      container_details: containerDetails.map((container) => ({
        container_type_input: String((container.container_type_input as string) || (container.container_type_name as string) || ""),
        container_no: String((container.container_no as string) || ""),
        actual_seal_no: String((container.actual_seal_no as string) || ""),
        customs_seal_no: String((container.customs_seal_no as string) || ""),
        loading_date: (container.loading_date as string) ? dayjs(container.loading_date as string).isValid() ? dayjs(container.loading_date as string).format("YYYY-MM-DD") : null : null,
        uploading_date: (container.uploading_date as string) ? dayjs(container.uploading_date as string).isValid() ? dayjs(container.uploading_date as string).format("YYYY-MM-DD") : null : null,
        cfs_id: Number((container.cfs_id as number)) || null,
      })),
      estimates: rateDetails.map((charge) => ({
        supplier_code: "",
        charge_id: Number((charge.charge_id as number)) || "",
        pp_cc: String((charge.pp_cc as string) || "Collect"),
        unit_id: Number((charge.unit_id as number)) || "",
        no_of_unit: Number(charge.no_of_unit) || 0,
        currency_id: Number((charge.currency_id as number)) || "",
        roe: Number((charge.roe as number)) || "",
        cost_per_unit: Number((charge.cost_per_unit as number)) || "",
        total_cost: Number((charge.total_cost as number)) || "",
      })),
    };
