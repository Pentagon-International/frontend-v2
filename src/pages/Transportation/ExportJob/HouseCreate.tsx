...
function HouseCreate() {
  ...
  const { isChaMode, basePath: jobModuleBasePath } = useJobModulePaths({
    ...
    listKey: "OCEAN_EXPORT_JOB_MASTER",
  ...
  useLayoutEffect(() => {
    if (!shouldFocusConsigneeFreeTextRef.current || !consigneeManualMode) return;
  ...
  }, [consigneeManualMode, consigneeSearch]);
  ...
  const addressesMatchForSelect = (a?: string, b?: string) =>
    String(a || "")
      .replace(/\s+/g, " ")
      .trim()
      .toLowerCase() ===
  ...
  const {
    ...
    activeTab: active,
    accountsTabIndex: 4,
    shipmentId: editData?.shipment_id,
  ...
  useEffect(() => {
    ...
    if (isEditMode && editData) {
      ...
      if ((editData as { forwarder_id?: number | null }).forwarder_id != null) {
        form.setFieldValue(
          "forwarder_id",
          Number((editData as { forwarder_id?: number | null }).forwarder_id),
        );
      }
      if ((editData as { forwarder_name?: string }).forwarder_name) {
        form.setFieldValue(
          "forwarder_name",
          String((editData as { forwarder_name?: string }).forwarder_name),
        );
      }
      if ((editData as { forwarder_address?: string }).forwarder_address) {
        const addr = String(
          (editData as { forwarder_address?: string }).forwarder_address,
        );
        form.setFieldValue("forwarder_address", addr);
      ...
      if ((editData as { forwarder_email?: string }).forwarder_email) {
        form.setFieldValue(
          "forwarder_email",
          String((editData as { forwarder_email?: string }).forwarder_email),
        );
      }

      if ((editData as { billing_customer_id?: number | null }).billing_customer_id != null) {
        form.setFieldValue(
          "billing_customer_id",
          Number((editData as { billing_customer_id?: number | null }).billing_customer_id),
        );
      }
      if ((editData as { billing_customer_name?: string }).billing_customer_name) {
        form.setFieldValue(
          "billing_customer_name",
          String((editData as { billing_customer_name?: string }).billing_customer_name),
        );
      }
      if ((editData as { billing_customer_address?: string }).billing_customer_address) {
        const addr = String(
          (editData as { billing_customer_address?: string }).billing_customer_address,
        );
        form.setFieldValue("billing_customer_address", addr);
      ...
      if ((editData as { billing_customer_email?: string }).billing_customer_email) {
        form.setFieldValue(
          "billing_customer_email",
          String((editData as { billing_customer_email?: string }).billing_customer_email),
        );
      }
      ...
      if (editData.shipper_address) {
        const addr = String(editData.shipper_address);
  ...
  const form = useForm<HouseDetailsForm>({
    initialValues: {
      ...
      forwarder_id:
        (editData as { forwarder_id?: number | null } | undefined)
          ?.forwarder_id != null
          ? Number(
              (editData as { forwarder_id?: number | null }).forwarder_id,
            )
          : null,
      forwarder_name:
        (editData as { forwarder_name?: string } | undefined)
          ?.forwarder_name || "",
      forwarder_address:
        (editData as { forwarder_address?: string } | undefined)
          ?.forwarder_address || "",
      forwarder_email:
  ...
  const buildUpdatedHousingDetailsFromForm = () => {
    ...
    const housingDetail = {
      ...
      forwarder_id: form.values.forwarder_id ?? null,
  ...
  const getCurrentHousingDetail = () => {
    ...
    return {
      ...
      forwarder_id: v.forwarder_id ?? null,
