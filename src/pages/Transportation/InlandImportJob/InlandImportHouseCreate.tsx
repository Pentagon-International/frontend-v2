...
function HouseCreate() {
  ...
  useEffect(() => {
    ...
    if (!formInitializedFromEditDataRef.current) {
      ...
      const savedAgentAddress = String(
        (editData as { agent_address?: string }).agent_address || "",
      );
      if (savedAgentAddress) {
        setAgentAddressOptions([
          {
            value: savedAgentAddress,
            label: savedAgentAddress,
            email: String(
              (editData as { agent_email?: string }).agent_email || "",
            ),
          },
        ]);
      }

      const savedForwarderAddress = String(
        (editData as { forwarder_address?: string }).forwarder_address || "",
      );
      if (savedForwarderAddress) {
        setForwarderAddressOptions([
          {
            value: savedForwarderAddress,
            label: savedForwarderAddress,
            email: String(
              (editData as { forwarder_email?: string }).forwarder_email || "",
            ),
          },
    ...
    const updatedCharges = chargesForm.values.charges.map((charge) => {
      ...
      if (
        ...
        const calculatedAmount =
          clampCurrencyMoneyAmountBound(noOfUnit * amountPerUnit) ?? 0;
  ...
  // Ensure trade is always "Import" for import pages
  useEffect(() => {
    if (form.values.trade !== "Import") {
      form.setFieldValue("trade", "Import");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.values.trade]);
  ...
  const buildUpdatedHousingDetailsFromForm = () => {
    ...
    const housingDetail = {
      ...
      forwarder_id: currentFormValues.forwarder_id ?? null,
  ...
  const handleOpenSendEmailForCan = () => {
    ...
    setActiveFileName(
      `Cargo-Arrival-Notice-${form.values.hawb_number || "AWB"}.pdf`,
    );
            ...
            <Grid>
              ...
              <Grid.Col span={4}>
                <Dropdown
                  ...
                  data={tradeOptions}
                  value={form.values.trade || "Import"}
                  disabled
                  onChange={() => {
                    // Prevent changes - trade is always "Import" for import pages
                    form.setFieldValue("trade", "Import");
                  }}
              ...
              <Grid.Col span={4}>
                <SearchableSelect
                  ...
                  onChange={(value, _selectedData, originalData) => {
                    const previousName = form.values.agent_name;
                    const previousAddress = form.values.agent_address;
                    const previousEmail = form.values.agent_email;
                    ...
                    if (
                      ...
                      const keepExisting =
                        !!previousAddress &&
                        (!previousName || previousName === newValue);
                      ...
                      setAgentAddressOptions(optionsWithSaved);
                      ...
                      if (matched) {
                        ...
                        if (previousEmail) {
                          form.setFieldValue("agent_email", previousEmail);
                        }
                      } else if (primaryAddr?.address) {
                        form.setFieldValue(
                          "agent_address",
                          primaryAddr.address,
                        );
          ...
          <Box mt="md">
                ...
                <Group gap="xs">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      navigate("/inland/import-job/credit-note", {
                        state: {
                          serviceType: "AIR",
            ...
            <Box mb="md">
              ...
              >
                ...
                <Grid.Col span={0.9}>
                  <RequiredLabel label="PP/CC" required />
                </Grid.Col>
                <Grid.Col span={1.2}>
