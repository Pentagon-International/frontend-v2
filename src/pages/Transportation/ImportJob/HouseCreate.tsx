...
function HouseCreate() {
  ...
  const [billingCustomerAddressOptions, setBillingCustomerAddressOptions] =
    useState<Array<{ value: string; label: string; email?: string }>>(() => {
      const ed = location.state?.editData as
        | { billing_customer_address?: string; billing_customer_email?: string }
        | undefined;
      const addr = ed?.billing_customer_address;
      if (!addr) return [];
      return [
        {
          value: String(addr),
          label: String(addr),
          email: String(ed?.billing_customer_email || ""),
        },
      ];
    });
  ...
  const form = useForm<HouseDetailsForm>({
    initialValues: {
      ...
      shipper_code: String(editData?.shipper_code || ""),
      ...
      shipper_state_id:
        editData?.shipper_state_id != null
          ...
          : "",
      consignee_code: String(editData?.consignee_code || ""),
  ...
  useEffect(() => {
    ...
    const shipperSelectValue = String(editData.shipper_code || "");
  ...
  const fillFormFromSimilarBooking = useCallback(() => {
    ...
    const rawHaz = (rb as { haz?: unknown; is_hazardous?: unknown }).haz ?? (rb as { is_hazardous?: unknown }).is_hazardous;
    const hazValue =
      rawHaz === true || rawHaz === "Yes"
        ? true
        : rawHaz === false || rawHaz === "No"
          ? false
          : null;
  ...
  useEffect(() => {
    const updatedCharges = chargesForm.values.charges.map((charge) => {
      ...
      // Calculate: cost_local_amount = total_cost * roe (0 allowed)
      if (
