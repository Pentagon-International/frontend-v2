...
function AirImportJobMaster() {
  ...
  } = useQuery<{ data: AirImportJobData[]; total_count: number }>({
    queryKey: ["airImportJobs"],
    queryFn: async () => {
      try {
        const response = await apiCallProtected.post(
          URL.filterJobCreate,
          { filters: { service: "AIR", service_type: "Import" } },
          API_HEADER
        );
        ...
        console.error("Error fetching air import jobs:", error);
        return { data: [], total_count: 0 };
      }
    },
  ...
  const data = useMemo(() => importJobData?.data || [], [importJobData]);
  const totalRecords = useMemo(
    () => importJobData?.total_count || 0,
    [importJobData]
  );
  ...
  useEffect(() => {
    if (location.state?.refreshData) {
      queryClient.invalidateQueries({ queryKey: ["airImportJobs"] });
      refetchImportJobs();
    ...
    <Card shadow="sm" padding="lg" radius="md" withBorder>
      <Group justify="space-between" align="center" mb="md" wrap="nowrap">
        ...
        <Button
          ...
          size="xs"
          color="#105476"
          onClick={() => navigate("/air/import-job/create")}
        >
          Create New
        </Button>
      </Group>
      ...
      {isLoading ? (
        <Center py="xl">
          <Stack align="center" gap="md">
            ...
            <Text c="dimmed">Loading air import jobs...</Text>
          </Stack>
        </Center>
      ) : (
